...
lrfdFHWAUHPCCreepCoefficient::lrfdFHWAUHPCCreepCoefficient()
{
}
...
Float64 lrfdFHWAUHPCCreepCoefficient::GetUltimateCreep() const
{
   return 1.2;
}

Float64 lrfdFHWAUHPCCreepCoefficient::GetAdjustedInitialAge(Float64 ti) const
{
   return ti;
}

Float64 lrfdFHWAUHPCCreepCoefficient::GetKl(Float64 ti) const
{
...
Float64 lrfdFHWAUHPCCreepCoefficient::ComputeKvs() const
{
   return 1.0;
}

Float64 lrfdFHWAUHPCCreepCoefficient::ComputeKhc() const
{
   return 1.12 - 0.0024*m_H;
}

Float64 lrfdFHWAUHPCCreepCoefficient::ComputeKf() const
{
   Float64 fci = WBFL::Units::ConvertFromSysUnits(m_Fci, WBFL::Units::Measure::KSI);
   return 18.0/(1.5*fci - 3.0);
}

Float64 lrfdFHWAUHPCCreepCoefficient::ComputeKtd(Float64 t) const
{
...
bool lrfdFHWAUHPCCreepCoefficient::TestMe(WBFL::Debug::Log& rlog)
{
   TESTME_PROLOGUE("lrfdFHWAUHPCCreepCoefficient");

   lrfdAutoVersion av;
   lrfdVersionMgr::SetUnits(lrfdVersionMgr::US);
   lrfdVersionMgr::SetVersion(lrfdVersionMgr::ThirdEditionWith2005Interims);

   // based on FHWA UHPC design example
   lrfdFHWAUHPCCreepCoefficient creep;
   ...
   TESTME_EPILOG("lrfdFHWAUHPCCreepCoefficient");
}
#endif // _UNITTEST
