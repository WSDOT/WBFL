...
BOOL CPointLoadListPage::OnInitDialog()
{
   ...
   for ( CollectionIndexType idx = 0; idx < nLoadings; idx++ )
   {
      ...
      for ( CollectionIndexType ldIdx = 0; ldIdx < nLoads; ldIdx++ )
      {
         ...
         m_ctrlList.InsertItem(idx,_T("Load"));

         CString str;
         str.Format(_T("%d"),ID);
         m_ctrlList.SetItemText((int)idx,0,str);

         str.Format(_T("%d"),mbrID);
         m_ctrlList.SetItemText((int)idx,1,str);

         str.Format(_T("%f"),location);
         m_ctrlList.SetItemText((int)idx,2,str);

         str.Format(_T("%s"),strOrientation[orientation]);
         m_ctrlList.SetItemText((int)idx,3,str);

         str.Format(_T("%d"),loadID);
         m_ctrlList.SetItemText((int)idx,4,str);

         str.Format(_T("%f"),fx);
         m_ctrlList.SetItemText((int)idx,5,str);

         str.Format(_T("%f"),fy);
         m_ctrlList.SetItemText((int)idx,6,str);

         str.Format(_T("%f"),mz);
         m_ctrlList.SetItemText((int)idx,7,str);
      }
   }
