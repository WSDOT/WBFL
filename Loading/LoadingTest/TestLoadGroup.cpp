...
void CTestLoadGroup::Test()
{
   ...
   hr = BSTRSetContainerTester<ldILoadGroup,ldILoadGroups,CTestLoadGroup,ldILoadGroupsEvents,&IID_ldILoadGroupsEvents, ldIEnumLoadGroup>::TestContainer(pMe,CLSID_ldLoadGroup,CLSID_ldLoadGroups);
   TRY_TEST(hr, S_OK);

   // test a LoadGroup by itself
   CComPtr<ldILoadGroup> pLoadGroup;
   TRY_TEST(pLoadGroup.CoCreateInstance( CLSID_ldLoadGroup ), S_OK );
   CComBSTR str;
   ...
   // let's listen to pcs
   DWORD dwCookie;
   CComQIPtr<ldILoadGroupEvents> events(pMe);
   CComPtr<IUnknown> punk(events);
   TRY_TEST(AtlAdvise(pLoadGroup,punk,IID_ldILoadGroupEvents,&dwCookie),S_OK);
...

   {
      ...
      CComPtr<ldILoadGroup> psm2;
      TRY_TEST(psm2.CoCreateInstance( CLSID_ldLoadGroup ), S_OK );
   ...
   TRY_TEST(AtlUnadvise(pLoadGroup,IID_ldILoadGroupEvents,dwCookie),S_OK);
   pMe->Release();
}
...
HRESULT CTestLoadGroup::OnLoadGroupChanged(ldILoadGroup* LoadGroup, ldLoadingChangeType change)
{
...
HRESULT CTestLoadGroup::OnLoadGroupsChanged(ldILoadGroup* LoadGroup, ldLoadingChangeType change)
{
...
HRESULT CTestLoadGroup::OnLoadGroupsAdded(/*[in]*/ldILoadGroup* load)
{
   m_csLoadGroupsFired = true;
   return S_OK;
}

HRESULT CTestLoadGroup::OnLoadGroupsBeforeRemove(/*[in]*/ldILoadGroup* load)
{
   m_csLoadGroupsFired = true;
   return S_OK;
}

HRESULT CTestLoadGroup::OnCreateSegment(ldILoadGroup* pLoadGroup)
{
   // give LoadGroup a unique name
   TCHAR str[45];
   int cOut = sprintf_s( str, 45, "LoadGroup %d", m_LastID++);
   ATLASSERT( cOut < 45 );
