...
template <class TISegment, class TIContainer, 
          class TListen, class TEvents, 
          const UUID* pEventUUID, class TIEnum>
class ContainerTester
{
public:
static HRESULT TestSegmentContainer(TListen* pMe,const CLSID& segmentCLSID, const CLSID& containerCLSID)
{
   AxleIndexType cnt;

   // create some segments
   CComPtr<TISegment> pss0, pss1, pss2,pss3,pss4, pss5, pss6;
   TRY_TEST( pss0.CoCreateInstance( segmentCLSID ) , S_OK );
   TRY_TEST( pMe->OnCreateSegment(pss0), S_OK);
   TRY_TEST( pss1.CoCreateInstance( segmentCLSID ) , S_OK );
   TRY_TEST( pMe->OnCreateSegment(pss1), S_OK);
   TRY_TEST( pss2.CoCreateInstance( segmentCLSID ) , S_OK );
   TRY_TEST( pMe->OnCreateSegment(pss2), S_OK);
   TRY_TEST( pss3.CoCreateInstance( segmentCLSID ) , S_OK );
   TRY_TEST( pMe->OnCreateSegment(pss3), S_OK);
   TRY_TEST( pss4.CoCreateInstance( segmentCLSID ) , S_OK );
   TRY_TEST( pMe->OnCreateSegment(pss4), S_OK);
   TRY_TEST( pss5.CoCreateInstance( segmentCLSID ) , S_OK );
   TRY_TEST( pMe->OnCreateSegment(pss5), S_OK);
   TRY_TEST( pss6.CoCreateInstance( segmentCLSID ) , S_OK );
...
//   HRESULT hr = pMe->TEvents::DispEventAdvise(pcnt1, pEventDSPID);
   // set up the listener for the container
   DWORD dwCookie;
   ...
   // stop listening
   // pMe->TEvents::DispEventUnadvise(pcnt1,pEventDSPID);
...
static HRESULT TestVectorContainer(TListen* pMe,const CLSID& segmentCLSID, const CLSID& containerCLSID)
{
