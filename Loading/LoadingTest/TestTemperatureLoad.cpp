...
void CTestTemperatureLoad::Test()
{
   ...
   hr = LoadContainerTester<ldITemperatureLoad,ldITemperatureLoadItem,ldITemperatureLoads,CTestTemperatureLoad,ldITemperatureLoadsEvents,&IID_ldITemperatureLoadsEvents, ldIEnumTemperatureLoad>::TestContainer(pMe,CLSID_ldTemperatureLoad,CLSID_ldTemperatureLoads);
   TRY_TEST(hr, S_OK);

   // first test a poi by itself
   CComPtr<ldITemperatureLoad> pTemperatureLoad;
   ...
   // let's listen to pcs
   DWORD dwCookie;
   CComQIPtr<ldITemperatureLoadEvents> events(pMe);
   CComPtr<IUnknown> punk(events);
   TRY_TEST(AtlAdvise(pTemperatureLoad,punk,IID_ldITemperatureLoadEvents,&dwCookie),S_OK);
   ...
   TRY_TEST( pTemperatureLoad->put_MemberID(-1), E_INVALIDARG );
...

   {
      ...
      CComPtr<ldITemperatureLoad> pTemperatureLoad2;
      TRY_TEST(pTemperatureLoad2.CoCreateInstance( CLSID_ldTemperatureLoad ), S_OK );
   ...
   CComPtr<ldITemperatureLoad> pclone;
   ...
   TRY_TEST(AtlUnadvise(pTemperatureLoad,IID_ldITemperatureLoadEvents,dwCookie),S_OK);
   pMe->Release();
}



HRESULT CTestTemperatureLoad::OnTemperatureLoadChanged(ldITemperatureLoad* TemperatureLoad)
{
...
HRESULT CTestTemperatureLoad::OnTemperatureLoadsChanged(ldITemperatureLoadItem* TemperatureLoad)
{
   m_csTemperatureLoadFired = true;
   return S_OK;
}
HRESULT CTestTemperatureLoad::OnTemperatureLoadsAdded(ldITemperatureLoadItem* TemperatureLoad)
{
   m_csTemperatureLoadFired = true;
   return S_OK;
}
HRESULT CTestTemperatureLoad::OnTemperatureLoadsBeforeRemove(ldITemperatureLoadItem* TemperatureLoad)
{
   m_csTemperatureLoadFired = true;
   return S_OK;
}
