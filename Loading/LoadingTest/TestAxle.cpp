...
void CTestAxle::Test()
{
   ...
   hr = ContainerTester<ldIAxle,ldIAxles,CTestAxle,ldIAxlesEvents,&IID_ldIAxlesEvents,ldIEnumAxle>::TestSegmentContainer(pMe,CLSID_ldAxle,CLSID_ldAxles);
   TRY_TEST(hr, S_OK);

   // test a Axle by itself
   CComPtr<ldIAxle> paxle;
   TRY_TEST(paxle.CoCreateInstance( CLSID_ldAxle ), S_OK );
   Float64 d;
   ...
   // let's listen to pcs
   DWORD dwCookie;
   CComQIPtr<ldIAxleEvents> events(pMe);
   CComPtr<IUnknown> punk(events);
   TRY_TEST(AtlAdvise(paxle,punk,IID_ldIAxleEvents,&dwCookie),S_OK);
...

   {
      ...
      CComPtr<ldIAxle> psm2;
      TRY_TEST(psm2.CoCreateInstance( CLSID_ldAxle ), S_OK );
   ...
   TRY_TEST(AtlUnadvise(paxle,IID_ldIAxleEvents,dwCookie),S_OK);
   pMe->Release();
}


HRESULT CTestAxle::OnAxleChanged(ldIAxle* Axle)
{
   m_csAxleFired = true;
...
HRESULT CTestAxle::OnAxlesChanged(ldIAxle* Axle)
{
   m_csAxleFired = true;
...
STDMETHODIMP CTestAxle::OnAxlesAdded(ldIAxle* item, AxleIndexType position)
{
   m_csAxlesFired = true;
   return S_OK;
}
STDMETHODIMP CTestAxle::OnAxlesBeforeRemove(ldIAxle* item, AxleIndexType position)
{
   m_csAxlesFired = true;
   return S_OK;
}
