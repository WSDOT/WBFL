...
void CTestPointLoad::Test()
{
   ...
   hr = LoadContainerTester<ldIPointLoad,ldIPointLoadItem, ldIPointLoads,CTestPointLoad,ldIPointLoadsEvents,&IID_ldIPointLoadsEvents, ldIEnumPointLoad>::TestContainer(pMe,CLSID_ldPointLoad,CLSID_ldPointLoads);
   TRY_TEST(hr, S_OK);

   // first test a poi by itself
   CComPtr<ldIPointLoad> pPointLoad;
   TRY_TEST(pPointLoad.CoCreateInstance( CLSID_ldPointLoad ), S_OK );
   CComBSTR bst;
   Float64 d;
   ...
   // let's listen to pcs
   DWORD dwCookie;
   CComQIPtr<ldIPointLoadEvents> events(pMe);
   CComPtr<IUnknown> punk(events);
   TRY_TEST(AtlAdvise(pPointLoad,punk,IID_ldIPointLoadEvents,&dwCookie),S_OK);
   ...
   TRY_TEST( pPointLoad->put_MemberID(-1), E_INVALIDARG );
...

   {
      ...
      CComPtr<ldIPointLoad> pPointLoad2;
      TRY_TEST(pPointLoad2.CoCreateInstance( CLSID_ldPointLoad ), S_OK );
   ...
   TRY_TEST(AtlUnadvise(pPointLoad,IID_ldIPointLoadEvents,dwCookie),S_OK);
   pMe->Release();
}



HRESULT CTestPointLoad::OnPointLoadChanged(ldIPointLoad* PointLoad)
{
...
HRESULT CTestPointLoad::OnPointLoadChanged(ldIPointLoadItem* PointLoad)
{
   m_csPointLoadFired = true;
   return S_OK;
}
HRESULT CTestPointLoad::OnPointLoadAdded(ldIPointLoadItem* PointLoad)
{
   m_csPointLoadFired = true;
   return S_OK;
}
HRESULT CTestPointLoad::OnPointLoadRemoved()
{
   m_csPointLoadFired = true;
   return S_OK;
}
