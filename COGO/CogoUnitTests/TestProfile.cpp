...
// TestProfile.cpp
//
// Ported from F:\ARP\WBFL\COGO\CogoTest\TestProfile.cpp (legacy WBFLTest.h
...
// Ported unchanged from F:\ARP\WBFL\COGO\CogoTest\TestProfile.cpp. Default
// argument values are carried over from that file's forward declarations
// (lines 37-39), which several call sites below rely on.
void TestElevation(IProfile* profile, IDType surfaceID,Float64 offset,Float64 results[],long minStation=440,long maxStation=1560,long inc=160)
{
         ...
         for ( long i = minStation; i <= maxStation; i += inc, j++)
         {
            ...
            ASSERT_EQ( IsEqual(elev, results[j],0.001), true );
         }
}
...
void TestGrade(IProfile* profile,Float64 results[],long minStation=440,long maxStation=1560,long inc=160)
{
         ...
         for ( long i = minStation; i <= maxStation; i += inc, j++)
         {
            ...
            ASSERT_EQ( IsEqual(grade*100, results[j],0.001), true );
         }
}
...
void TestCrownSlopes(IProfile* profile,IDType surfaceID,IndexType ridgePointIdx,Float64 results[][2],long minStation=440,long maxStation=1560,long inc=160)
{
         ...
         for ( long i = minStation; i <= maxStation; i += inc, j++)
         {
            ...
            ASSERT_EQ( IsEqual(left,  results[j][0], 0.001), true );
            ASSERT_EQ( IsEqual(right, results[j][1], 0.001), true );
         }
}
...
public:
   TEST_METHOD(Test1)
   {
         ...
         ASSERT_EQ(profile.CoCreateInstance(CLSID_Profile),S_OK);
         ...
         ASSERT_EQ(profile->AddProfileElement(nullptr),E_INVALIDARG);
         ASSERT_EQ(profile->AddProfileElement(element), S_OK);
         ...
         //
         IndexType count;
         ASSERT_EQ(profile->get_ProfileElementCount(nullptr),E_POINTER);
         ASSERT_EQ(profile->get_ProfileElementCount(&count),S_OK);
         ASSERT_EQ(count,2);
         ...
         element.Release();
         ASSERT_EQ(profile->get_Item(-1,&element),E_INVALIDARG);
         ASSERT_EQ(profile->get_Item(100,&element),E_INVALIDARG);
         ASSERT_EQ(profile->get_Item(0,nullptr),E_POINTER);
         ASSERT_EQ(profile->get_Item(0,&element),S_OK);
         ...
         station->get_Value(&sta);
         point->get_Elevation(&elev);
         ASSERT_EQ(IsEqual(sta, 50.0), true);
         ASSERT_EQ(IsEqual(elev, 10.0), true);

         element.Release();
         ASSERT_EQ(profile->get_Item(1,&element),S_OK);
         ...
         ASSERT_EQ(IsEqual(elev,250.0),true);
         ...
         //point.Release();
         //vc->get_BVC(&point);
         //ASSERT_EQ(profile->Remove(CComVariant()),E_INVALIDARG);
         //ASSERT_EQ(profile->Remove(CComVariant(profile)),E_INVALIDARG);
         //ASSERT_EQ(profile->Remove(CComVariant(-1)),E_INVALIDARG);
         //ASSERT_EQ(profile->Remove(CComVariant(100)),E_INVALIDARG);
         //ASSERT_EQ(profile->Remove(CComVariant(point)),E_INVALIDARG);
         //ASSERT_EQ(profile->Remove(CComVariant(vc)),S_OK); // Remove the vert curve
         //ASSERT_EQ(profile->Remove(CComVariant(1)),S_OK);  // Remove the element at index 1
         ...
         //ASSERT_EQ(profile->get__EnumProfileElements(nullptr), E_POINTER );
         //ASSERT_EQ(profile->get__EnumProfileElements(&pEnum), S_OK );

         //ULONG fetched;
         //CComPtr<IProfileElement> pe;
         //ASSERT_EQ(pEnum->Next(1,&pe,&fetched ),S_OK);
         //ASSERT_EQ(pe.IsEqualObject(element),true);
         ...
         ASSERT_EQ( eInfo->InterfaceSupportsErrorInfo( IID_IProfile ), S_OK );
         ASSERT_EQ( eInfo->InterfaceSupportsErrorInfo( IID_ISupportErrorInfo ), S_FALSE );

         // Test IObjectSafety
         ASSERT_EQ( TestIObjectSafety(CLSID_Profile,IID_IProfile,INTERFACESAFE_FOR_UNTRUSTED_CALLER | INTERFACESAFE_FOR_UNTRUSTED_DATA), true);
   }
   ...
   TEST_METHOD(Test7)
   {
         ...
         ASSERT_EQ(IsEqual(val, 0.02), true);
         ...
         ASSERT_EQ(IsEqual(val, 0.03), true);

         profile->CrossSlope(surfaceID, CComVariant(75.00), 10.0, &val);
         ASSERT_EQ(IsEqual(val, -0.03), true);

         profile->CrossSlope(surfaceID, CComVariant(100.00), -10.0, &val);
         ASSERT_EQ(IsEqual(val, 0.04), true);
   ...
   TEST_METHOD(Test9)
   {
         ...
         Float64 elev;
         profile->Elevation(INVALID_ID, CComVariant(4700), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 437.84), true);
         profile->Elevation(INVALID_ID, CComVariant(4850), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 436.34), true);
         profile->Elevation(INVALID_ID, CComVariant(4900), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 435.89), true);
         profile->Elevation(INVALID_ID, CComVariant(5000), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 435.29), true);
         profile->Elevation(INVALID_ID, CComVariant(5100), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 435.09), true);
         profile->Elevation(INVALID_ID, CComVariant(5200), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 435.29), true);
         profile->Elevation(INVALID_ID, CComVariant(5300), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 435.89), true);
         profile->Elevation(INVALID_ID, CComVariant(5400), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 436.89), true);
         profile->Elevation(INVALID_ID, CComVariant(5500), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 438.29), true);
         profile->Elevation(INVALID_ID, CComVariant(5600), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 440.09), true);
         profile->Elevation(INVALID_ID, CComVariant(5650), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 441.14), true);
         profile->Elevation(INVALID_ID, CComVariant(6000), 0.00,&elev);   ASSERT_EQ(IsEqual(elev,448.84),true);
   }
   ...
   TEST_METHOD(Test10)
   {
         ...
         Float64 elev;
         profile->Elevation(surfaceID, CComVariant(100), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 100.00), true);
         profile->Elevation(surfaceID, CComVariant(100), 5.00, &elev);   ASSERT_EQ(IsEqual(elev, 99.80), true);
         profile->Elevation(surfaceID, CComVariant(100), -5.00,&elev);   ASSERT_EQ(IsEqual(elev, 99.90),true);
   }
   ...
   TEST_METHOD(Test11)
   {
         ...
         Float64 elev;
         profile->Elevation(INVALID_ID, CComVariant(250), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 72.5), true);
         profile->Elevation(INVALID_ID, CComVariant(450), 0.00, &elev);   ASSERT_EQ(IsEqual(elev, 57.5), true);
         profile->Elevation(INVALID_ID, CComVariant(650), 0.00,&elev);   ASSERT_EQ(IsEqual(elev,72.5),true);
   }
   ...
   TEST_METHOD(Test12)
   {
         ...
         for (int i = 0; i < 50; i++)
         {
            ...
            ASSERT_EQ(IsEqual(station, std::get<0>(vResults1[i])),true);
            ASSERT_EQ(IsEqual(std::get<1>(vResults1[i]), elevL),true);
            ASSERT_EQ(IsEqual(std::get<2>(vResults1[i]), elevP),true);
            ASSERT_EQ(IsEqual(std::get<3>(vResults1[i]), elevR),true);
         }
         ...
         for (int i = 0; i < 50; i++)
         {
            ...
            ASSERT_EQ(IsEqual(station, std::get<0>(vResults2[i])), true);
            ASSERT_EQ(IsEqual(std::get<1>(vResults2[i]), elevL), true);
            ASSERT_EQ(IsEqual(std::get<2>(vResults2[i]), elevP), true);
            ASSERT_EQ(IsEqual(std::get<3>(vResults2[i]), elevR), true);
         }
         ...
         for (int i = 0; i < 50; i++)
         {
            ...
            ASSERT_EQ(IsEqual(station, std::get<0>(vResults3[i])), true);
            ASSERT_EQ(IsEqual(std::get<1>(vResults3[i]), elevL), true);
            ASSERT_EQ(IsEqual(std::get<2>(vResults3[i]), elevP), true);
            ASSERT_EQ(IsEqual(std::get<3>(vResults3[i]), elevR), true);
         }
   }
};
} // namespace CogoUnitTests