///////////////////////////////////////////////////////////////////////
// COGOTest - Test Driver for Coordinate Geometry Library
// Copyright � 1999-2026  Washington State Department of Transportation
//                        Bridge and Structures Office
//
// This library is a part of the Washington Bridge Foundation Libraries
// and was developed as part of the Alternate Route Project
//
// This program is free software; you can redistribute it and/or modify
// it under the terms of the Alternate Route Open Source License as 
// published by the Washington State Department of Transportation,
// Bridge and Structures Office.
//
// This program is distributed in the hope that it will be useful,
// but is distributed AS IS, WITHOUT ANY WARRANTY; without even the
// implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR 
// PURPOSE.  See the Alternate Route Library Open Source License for more details.
//
// You should have received a copy of the Alternate Route Open Source License
// along with this program; if not, write to the Washington State
// Department of Transportation, Bridge and Structures Office,
// P.O. Box 47340, Olympia, WA 98503, USA or e-mail
// Bridge_Support@wsdot.wa.gov
///////////////////////////////////////////////////////////////////////

// TestMeasure.cpp
//
// Ported from F:\ARP\WBFL\COGO\CogoTest\TestMeasure.cpp (legacy WBFLTest.h
// harness) to MSVC CppUnitTestFramework, talking to WBFLCogo.dll purely
// through its public COM interfaces (see CogoTestUtil.h).
#include "pch.h"

namespace CogoUnitTests
{

TEST_CLASS(TestMeasure)
{
public:
   TEST_METHOD(BasicOperations)
   {
         CComPtr<IMeasure> measure;
         ASSERT_EQ(measure.CoCreateInstance(CLSID_CogoModel),S_OK);

         CComQIPtr<ICogoModel> model(measure);
         ASSERT_EQ( model != nullptr, true );

         model->StorePoint(1,10.0,10.0);
         model->StorePoint(2,40.0,20.0);
         model->StorePoint(3,40.0, 0.0);

         // Test Distance
         Float64 dist;
         ASSERT_EQ(measure->Distance(1,2,nullptr),E_POINTER);
         ASSERT_EQ(measure->Distance(-1,2,&dist),E_FAIL);
         ASSERT_EQ(measure->Distance(1,-2,&dist), E_FAIL);
         ASSERT_EQ(measure->Distance(1,2,&dist),S_OK);
         ASSERT_EQ(IsEqual(dist,31.62277660),true);

         ASSERT_EQ(measure->Distance(1,1,&dist),S_OK);
         ASSERT_EQ(IsEqual(dist,0.0),true);

         // Test Angle
         // Angle is implemented by WBFLGeometry...
         // All we need here is a simple integration test
         model->ClearPoints();
         model->StorePoint(1,10.0, 0.0);
         model->StorePoint(2, 0.0, 0.0);
         model->StorePoint(3, 0.0,10.0);

         CComPtr<IAngle> angle;
         Float64 val;
         ASSERT_EQ(measure->Angle(2,1,3,nullptr),E_POINTER);
         ASSERT_EQ(measure->Angle(-2,1,3,&angle), E_INVALIDARG);
         ASSERT_EQ(measure->Angle(2,-1,3,&angle), E_INVALIDARG);
         ASSERT_EQ(measure->Angle(2,1,-3,&angle), E_INVALIDARG);
         ASSERT_EQ(measure->Angle(2,1,1,&angle), E_INVALIDARG);
         ASSERT_EQ(measure->Angle(2,2,3,&angle), E_INVALIDARG);
         ASSERT_EQ(measure->Angle(1,1,3,&angle), E_INVALIDARG);

         ASSERT_EQ(measure->Angle(1,2,3,&angle),S_OK);
         angle->get_Value(&val);
         ASSERT_EQ(IsEqual(val,PI_OVER_2),true);

         angle.Release();
         ASSERT_EQ(measure->Angle(3,2,1,&angle),S_OK);
         angle->get_Value(&val);
         ASSERT_EQ(IsEqual(val,3*PI_OVER_2),true);

         //
         // Test Direction
         //
         model->ClearPoints();
         model->StorePoint(1,10.0,10.0);
         model->StorePoint(2,20.0,20.0);
         CComPtr<IDirection> dir;
         ASSERT_EQ(measure->Direction( 1,2,nullptr),E_POINTER);
         ASSERT_EQ(measure->Direction(-1,2,&dir), E_FAIL);
         ASSERT_EQ(measure->Direction(1,-2,&dir), E_FAIL);
         ASSERT_EQ(measure->Direction( 1,2,&dir),S_OK);
         NSDirectionType ns;
         EWDirectionType ew;
         long deg, min;
         Float64 sec;
         dir->get_NSDirection(&ns);
         dir->get_Degree(&deg);
         dir->get_Minute(&min);
         dir->get_Second(&sec);
         dir->get_EWDirection(&ew);
         ASSERT_EQ(ns,nsNorth);
         ASSERT_EQ(deg,45);
         ASSERT_EQ(min,0);
         ASSERT_EQ(IsEqual(sec,0.0),true);
         ASSERT_EQ(ew,ewEast);

         dir.Release();
         ASSERT_EQ(measure->Direction(2,1,&dir),S_OK);
         dir->get_NSDirection(&ns);
         dir->get_Degree(&deg);
         dir->get_Minute(&min);
         dir->get_Second(&sec);
         dir->get_EWDirection(&ew);
         ASSERT_EQ(ns,nsSouth);
         ASSERT_EQ(deg,45);
         ASSERT_EQ(min,0);
         ASSERT_EQ(IsEqual(sec,0.0),true);
         ASSERT_EQ(ew,ewWest);

         // 2 points at the same location should produce a bearing that is due east
         dir.Release();
         ASSERT_EQ(measure->Direction( 1,1,&dir), S_OK);
         dir->get_NSDirection(&ns);
         dir->get_Degree(&deg);
         dir->get_Minute(&min);
         dir->get_Second(&sec);
         dir->get_EWDirection(&ew);
         ASSERT_EQ(ns,nsNorth);
         ASSERT_EQ(deg,90);
         ASSERT_EQ(min,0);
         ASSERT_EQ(IsEqual(sec,0.0),true);
         ASSERT_EQ(ew,ewEast);

         // Test Area
         model->ClearPoints();
         model->StorePoint(1,0,0);
         model->StorePoint(2,10,0);
         model->StorePoint(3,10,10);
         model->StorePoint(4,0,10);

         SAFEARRAY* keys;
         SAFEARRAYBOUND bounds = { 4, 10 };
         keys = SafeArrayCreate(VT_I4,1,&bounds);
         for ( LONG i = bounds.lLbound; i < (LONG)(bounds.lLbound+bounds.cElements); i++ )
         {
            IDType key = IDType(i - bounds.lLbound + 1);
            SafeArrayPutElement(keys,&i,&key);
         }

         Float64 area;
         CComVariant varKeys;
         varKeys.vt = VT_ARRAY | VT_I4;
         varKeys.parray = keys;
         ASSERT_EQ(measure->Area(varKeys,nullptr),E_POINTER);
         ASSERT_EQ(measure->Area(varKeys,&area),S_OK);
         ASSERT_EQ(IsEqual(area,100.),true);
         SafeArrayDestroy(keys);

         // Try an array with undefined points
         keys = SafeArrayCreate(VT_I4,1,&bounds);
         for ( LONG i = bounds.lLbound; i < (LONG)(bounds.lLbound+bounds.cElements); i++ )
         {
            IDType key = IDType(i - bounds.lLbound + 1);
            if ( (i-bounds.lLbound) == 2 )
               key = 400;

            SafeArrayPutElement(keys,&i,&key);
         }
         varKeys.vt = VT_ARRAY | VT_I4;
         varKeys.parray = keys;
         ASSERT_EQ(measure->Area(varKeys,&area), E_FAIL);
         SafeArrayDestroy(keys);

         // Try an array with less than 3 elements
         bounds.cElements = 2;
         keys = SafeArrayCreate(VT_I4,1,&bounds);
         for ( LONG i = bounds.lLbound; i < (LONG)(bounds.lLbound+bounds.cElements); i++ )
         {
            IDType key = IDType(i - bounds.lLbound + 1);
            SafeArrayPutElement(keys,&i,&key);
         }
         varKeys.vt = VT_ARRAY | VT_I4;
         varKeys.parray = keys;
         ASSERT_EQ(measure->Area(varKeys,&area),E_INVALIDARG);
      //   SafeArrayDestroy(keys); // This is causing the test code to crash. I don't know why.
                                   // Since this is a test driver, who cares if it leaks a little
                                   // memory. Time to move on. RAB: 11/14/2001
   }
};
} // namespace CogoUnitTests