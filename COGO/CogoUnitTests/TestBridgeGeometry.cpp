...
// TestBridgeGeometry.cpp
//
// Ported from F:\ARP\WBFL\COGO\CogoTest\TestBridgeGeometry.cpp (legacy
// WBFLTest.h harness, class CTestBridgeFramingGeometry) to MSVC
// CppUnitTestFramework, talking to WBFLCogo.dll purely through its public
// COM interfaces (see CogoTestUtil.h).
#include "pch.h"

namespace CogoUnitTests
{

// Ported from F:\ARP\WBFL\COGO\CogoTest\TestBridgeGeometry.cpp. The legacy
// free functions TestPoint()/TestDirection() are renamed CheckPoint()/
// CheckDirection() here since TEST_CLASS(TestPoint)/TEST_CLASS(TestDirection)
// already exist as types in this namespace (ported from TestPoint.cpp /
// TestDirection.cpp) and would collide with same-named free functions.
...
HRESULT CreateBridgeGeometry(const std::vector<std::tuple<IDType,Float64,BSTR>>& vPiers,VARIANT_BOOL bContinuous, IBridgeGeometry** ppBridge)
{
         ...
         ASSERT_EQ(bridge.CoCreateInstance(CLSID_BridgeGeometry), S_OK);
         ...
         for (const auto& pier : vPiers)
         {
            ...
            ASSERT_EQ(factory.CoCreateInstance(CLSID_SinglePierLineFactory), S_OK);
         ...
         ASSERT_EQ(layout_line_factory.CoCreateInstance(CLSID_AlignmentOffsetLayoutLineFactory), S_OK);
         ...
         ASSERT_EQ(girder_line_factory.CoCreateInstance(CLSID_SimpleGirderLineFactory), S_OK);
...
bool CheckPoint(IPoint2d* pPoint, Float64 x, Float64 y)
{
         Float64 _x, _y;
...
bool CheckDirection(IDirection* pDirection, Float64 direction)
{
         Float64 value;
         pDirection->get_Value(&value);
         return IsEqual(value, direction);
}
TEST_CLASS(TestBridgeGeometry)
{
public:
   TEST_METHOD(Test1)
   {
         std::vector<std::tuple<IDType, Float64, BSTR>> piers;
         piers.emplace_back(100, 100.0, CComBSTR("N 45 W").Detach());
         piers.emplace_back(200, 200.0, CComBSTR("NORMAL").Detach());
         CComPtr<IBridgeGeometry> bridge;
         CreateBridgeGeometry(piers, VARIANT_TRUE, &bridge);

         IndexType nGirderLines;
         bridge->get_GirderLineCount(&nGirderLines);
         ASSERT_EQ(nGirderLines, 5);

         CComPtr<IPierLine> pier1, pier2;
         ASSERT_EQ(bridge->FindPierLine(100, &pier1), S_OK);
         ASSERT_EQ(bridge->FindPierLine(200, &pier2), S_OK);

         CComPtr<IPoint2d> point;
         pier1->get_AlignmentPoint(&point);
         ASSERT_EQ(CheckPoint(point, 80.710678, 80.710678), true);

         point.Release();
         pier2->get_AlignmentPoint(&point);
         ASSERT_EQ(CheckPoint(point, 151.421356, 151.421356), true);

         CComPtr<IDirection> direction;
         pier1->get_Direction(&direction);
         ASSERT_EQ(CheckDirection(direction, 3 * M_PI / 4), true);
         direction.Release();
         ...
         ASSERT_EQ(IsEqual(length, 100.0), true);
         ...
         ASSERT_EQ(CheckPoint(point, 80.710678 + 2.5 * cos(M_PI / 4), 80.710678 + 2.5 * sin(M_PI / 4)), true);
         point.Release();
         girder_line->get_BearingPoint(etEnd, &point);
         ASSERT_EQ(CheckPoint(point, 151.421356 - 2.5 * cos(M_PI / 4), 151.421356 - 2.5 * cos(M_PI / 4)), true);

         point.Release();
         girder_line->get_EndPoint(etStart, &point);
         ASSERT_EQ(CheckPoint(point, 80.710678 + 1.5 * cos(M_PI / 4), 80.710678 + 1.5 * sin(M_PI / 4)), true);
         point.Release();
         girder_line->get_EndPoint(etEnd, &point);
         ASSERT_EQ(CheckPoint(point, 151.421356 - 1.5 * cos(M_PI / 4), 151.421356 - 1.5 * cos(M_PI / 4)), true);
   }
   ...
   TEST_METHOD(Test2)
   {
         ...
         piers.emplace_back(100, 100.0, CComBSTR("N 45 W").Detach());
         piers.emplace_back(200, 200.0, CComBSTR("NORMAL").Detach());
         piers.emplace_back(300, 300.0, CComBSTR("NORMAL").Detach());
         CComPtr<IBridgeGeometry> bridge;
         CreateBridgeGeometry(piers, VARIANT_FALSE, &bridge);

         IndexType nGirderLines;
         bridge->get_GirderLineCount(&nGirderLines);
         ASSERT_EQ(nGirderLines, 10);
   }
   ...
   TEST_METHOD(Test3)
   {
         ...
         ASSERT_EQ(layout_line_factory_span_1.CoCreateInstance(CLSID_UniformSpacingLayoutLineFactory), S_OK);
         ...
         ASSERT_EQ(layout_line_factory_span_2.CoCreateInstance(CLSID_UniformSpacingLayoutLineFactory), S_OK);
         ...
         ASSERT_EQ(girder_line_factory_span_1.CoCreateInstance(CLSID_SimpleGirderLineFactory), S_OK);
         ...
         ASSERT_EQ(girder_line_factory_span_2.CoCreateInstance(CLSID_SimpleGirderLineFactory), S_OK);
   ...
   TEST_METHOD(Test4)
   {
         ...
         ASSERT_EQ(span_1_diaphragm_line_factory.CoCreateInstance(CLSID_EqualSpacingDiaphragmLineFactory), S_OK);
         ...
         ASSERT_EQ(span_2_diaphragm_line_factory.CoCreateInstance(CLSID_SingleDiaphragmLineFactory), S_OK);
         ...
         ASSERT_EQ(pier_2_diaphragm_line_factory.CoCreateInstance(CLSID_ThroughPointDiaphragmLineFactory), S_OK);
         CComPtr<IPoint2d> point;
         ...
         ASSERT_EQ(nDiaphragmLines, 5); // 3 in span 1, and 1 in spans 2 and 3
   ...
   {
         std::vector<std::tuple<IDType, Float64, BSTR>> piers;
         piers.emplace_back(100, 100.0, CComBSTR("45 L").Detach());
         piers.emplace_back(200, 200.0, CComBSTR("45 L").Detach());
         piers.emplace_back(300, 300.0, CComBSTR("45 L").Detach());
         CComPtr<IBridgeGeometry> bridge;
         CreateBridgeGeometry(piers, VARIANT_FALSE, &bridge);
         ...
         ASSERT_EQ(deck_edge_layout_line_factory.CoCreateInstance(CLSID_AlignmentOffsetLayoutLineFactory), S_OK);
         ...
         ASSERT_EQ(break_line_layout_line_factory.CoCreateInstance(CLSID_AlignmentOffsetLayoutLineFactory), S_OK);
         ...
         ASSERT_EQ(deck_boundary_factory.CoCreateInstance(CLSID_SimpleDeckBoundaryFactory), S_OK);
         ...
         ASSERT_EQ(bridge->get_DeckBoundary(&deck_boundary),S_OK);
         CComPtr<IPoint2dCollection> points;
         deck_boundary->get_Perimeter(10, &points);
         IndexType nPoints;
         points->get_Count(&nPoints);
         ASSERT_EQ(nPoints,26);
         ...
         for (IndexType i = 0; i < nPoints; i++)
         {
            ...
            ASSERT_EQ(CheckPoint(point, expected_points[i].first, expected_points[i].second), true);
         }
   }
};
} // namespace CogoUnitTests