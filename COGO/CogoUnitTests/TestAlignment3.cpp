...
// TestAlignment3.cpp
//
// Ported from F:\ARP\WBFL\COGO\CogoTest\TestAlignment3.cpp (legacy WBFLTest.h
...
// Ported unchanged from F:\ARP\WBFL\COGO\CogoTest\TestAlignment3.cpp (was a
// private static method of CTestAlignment3; ported as a free function since
// TEST_METHODs don't share a meaningful base class here).
void CompareAlignments(IAlignment* pAlignment1,IAlignment* pAlignment2,Float64 start,Float64 end,long nPoints)
{
         Float64 x1,y1;
         Float64 x2,y2;
         for ( long i = 0; i < nPoints; i++ )
         {
            ...
            ASSERT_EQ(IsEqual(x1,x2,0.2),true);
            ASSERT_EQ(IsEqual(y1,y2,0.2),true);
            ...
            d1->get_Value(&value1);
            d2->get_Value(&value2);
            ASSERT_EQ(IsEqual(value1,value2,0.2),true);
         }
}
TEST_CLASS(TestAlignment3)
{
public:
   TEST_METHOD(Test1)
   {
         ...
         ASSERT_EQ(alignment->CreateSubAlignment(CComVariant(-10.0),CComVariant(300),nullptr), E_POINTER);
         ASSERT_EQ(alignment->CreateSubAlignment(CComVariant(-10.0),CComVariant(300),&subAlignment), S_OK);

         IndexType nElements;
         subAlignment->get_Count(&nElements);

         ASSERT_EQ(nElements,3);

         CComPtr<IPathElement> element;
         subAlignment->get_Item(0,&element);

         CComQIPtr<IPathSegment> segment(element);
         ASSERT_EQ(segment != nullptr, true);
         ...
         ASSERT_EQ(alignment->CreateSubAlignment(CComVariant(-10.0),CComVariant(-5.0),&subAlignment),S_OK);

         subAlignment->get_Count(&nElements);

         ASSERT_EQ(nElements,1);
         ...
         subAlignment.Release();
         ASSERT_EQ(alignment->CreateSubAlignment(CComVariant(110.0),CComVariant(200.0),&subAlignment),S_OK);
   ...
   TEST_METHOD(Test2)
   {
         ...
         start->Location(&x, &y);
         ASSERT_EQ(IsEqual(x, -67.781745930520231), true);
         ASSERT_EQ(IsEqual(y, -67.781745930520231), true);

         end->Location(&x, &y);
         ASSERT_EQ(IsEqual(x, -64.246212024587493), true);
         ASSERT_EQ(IsEqual(y, -64.246212024587493), true);
         ...
         start->Location(&x, &y);
         ASSERT_EQ(IsEqual(x, 17.071067811865476), true);
         ASSERT_EQ(IsEqual(y, 17.071067811865476), true);

         end->Location(&x, &y);
         ASSERT_EQ(IsEqual(x, 80.710678118654755), true);
         ASSERT_EQ(IsEqual(y, 80.710678118654755), true);
   ...
   TEST_METHOD(Test3)
   {
         ...
         ASSERT_EQ( IsEqual(value,500.), true);

         objHC->get_SpiralLength(spEntry,&value);
         ASSERT_EQ( IsEqual(value,20.), true);

         objHC->get_SpiralLength(spExit,&value);
         ASSERT_EQ( IsEqual(value,0.), true);
