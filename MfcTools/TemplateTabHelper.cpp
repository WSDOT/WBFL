...
//======================== LIFECYCLE  =======================================
mfcTemplateTabHelper::mfcTemplateTabHelper(const std::string& name, const std::string& suffix):
m_Name(name),
m_Suffix(suffix)
{
}
...
void mfcTemplateTabHelper::GetFileList(TabDisplayFileList* pFileList)
{
   ...
   for (std::set<std::string>::iterator it=m_DirectoryList.begin(); it!=m_DirectoryList.end(); it++)
   {
      CString filespec = CString(it->c_str()) + CString("*.") + CString(m_Suffix.c_str());
      CFileFind finder;
      BOOL is_file;
      ...
      while(is_file)
      {
         ...
         TabFile file;
         file.m_FileName = std::string(finder.GetFileName());
         ...
         if (pib.second)
         {
            ...
            mf.m_LastWriteDate= tim.Format("%m/%d/%Y %I:%M:%S%p");
            mf.m_SizeInBytes.Format("%d bytes",finder.GetLength());
...
std::string mfcTemplateTabHelper::GetFilePath(const std::string& fileName)
{
   TabFile file;
   ...
   {
      return std::string(*(it->m_pDirectory) + fileName);
   }
   else
...
void mfcTemplateTabHelper::AddSubDirectory(const std::string& dirName) 
{
...
void mfcTemplateTabHelper::Dump(dbgDumpContext& os) const
{
   ...
   for (std::set<std::string>::const_iterator itd = m_DirectoryList.begin(); itd!=m_DirectoryList.end(); itd++)
   {
      os << "     "<<*itd<<endl;
   }
