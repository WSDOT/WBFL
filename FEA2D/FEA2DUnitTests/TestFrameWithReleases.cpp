...
//
// Mirrors Fem2d\Fem2dUnitTests\TestFrameWithReleases.cpp. The legacy file
// left most POI-level deflection/force assertions commented out (evidently
// disabled during development) for the cantilever sub-test; only the active
// (uncompiled-out) assertions are ported here to preserve behavior exactly.
#include "pch.h"
...
public:
   TEST_METHOD(SimpleCantilever_ThreeLoadCases_SupportConfigVariants)
   {
      ...
      model.CreatePOI(200, 1, 0.00);
      model.CreatePOI(201, 1, -0.25);
      model.CreatePOI(202, 1, -0.50);
      model.CreatePOI(203, 1, -0.75);
      model.CreatePOI(204, 1, -1.00);
   ...
   // in Fem2d\Test\Supporting Documents\WinStrudl\Releases.*).
   //             w
   //        ============ 3
   //                   /o
   //                 /  |
   ...
   TEST_METHOD(PortalFrameWithReleases_WinStrudlVerified)
   {
      ...
      model.CreateJoint(3, 10.0, 20.0);
      model.CreateJoint(4, 10.0, 10.0);
      model.CreateJoint(5, 10.0, 0.0);
      ...
      model.CreatePOI(200, 2, 0.00);
      model.CreatePOI(201, 2, -0.25);
      model.CreatePOI(202, 2, -0.50);
      model.CreatePOI(203, 2, -0.75);
      model.CreatePOI(204, 2, -1.00);
      ...
      Loading& pLoading0 = model.CreateLoading(0);
      pLoading0.CreatePointLoad(1, 2, -0.5, 0.0, -10.0, 0.0, LoadOrientation::Global);
      pLoading0.CreateJointLoad(1, 2, 10.0, 0.0, 0.0);

      // load case 1: uniform distributed load along member 2
      Loading& pLoading1 = model.CreateLoading(1);
      pLoading1.CreateDistributedLoad(1, 2, LoadDirection::Fy, 0.0, -1.0, -1.0, -1.0, LoadOrientation::Member);
      pLoading1.CreateJointLoad(1, 2, 10.0, 0.0, 0.0);

      // load case 2: temperature strain load along member 2
      Loading& pLoading2 = model.CreateLoading(2);
      pLoading2.CreateMemberStrain(1, 2, 0.0, -1.0, 2, 1);
      pLoading2.CreateJointLoad(1, 2, 10.0, 0.0, 0.0);
      ...
      model.ComputePOIDeflections(lc, 200, LoadOrientation::Global, &dx, &dy, &rz);
      ASSERT_TRUE(IsEqual(dx, 0.27095097726495));
      ASSERT_TRUE(IsEqual(dy, -0.55948045530510));
      ASSERT_TRUE(IsEqual(rz, -0.020546367819410));
      model.ComputePOIDeflections(lc, 201, LoadOrientation::Global, &dx, &dy, &rz);
      ASSERT_TRUE(IsEqual(dx, 0.18895930338732));
      ASSERT_TRUE(IsEqual(dy, -0.74146147387478));
      ASSERT_TRUE(IsEqual(rz, -0.018900871176406));
      model.ComputePOIDeflections(lc, 202, LoadOrientation::Global, &dx, &dy, &rz);
      ASSERT_TRUE(IsEqual(dx, 0.098740146294668));
      ASSERT_TRUE(IsEqual(dy, -0.91521500922943));
      ASSERT_TRUE(IsEqual(rz, -0.013964381247396));
      model.ComputePOIDeflections(lc, 203, LoadOrientation::Global, &dx, &dy, &rz);
      ASSERT_TRUE(IsEqual(dx, 0.17252556588730));
      ASSERT_TRUE(IsEqual(dy, -0.89941973067610));
      ASSERT_TRUE(IsEqual(rz, -0.010156315414796));
      model.ComputePOIDeflections(lc, 204, LoadOrientation::Global, &dx, &dy, &rz);
      ASSERT_TRUE(IsEqual(dx, 0.24372562274697));
      ASSERT_TRUE(IsEqual(dy, -0.88103908938980));
      ASSERT_TRUE(IsEqual(rz, -0.011896091061020));

      Float64 fx, fy, mz;
      model.ComputePOIForces(lc, 200, MemberFaceType::Left, LoadOrientation::Member, &fx, &fy, &mz);
      ASSERT_TRUE(IsEqual(fx, -5.2794538489462));
      ASSERT_TRUE(IsEqual(fy, -2.6327946288053));
      ASSERT_TRUE(IsEqual(mz, 0.0));
      model.ComputePOIForces(lc, 202, MemberFaceType::Left, LoadOrientation::Member, &fx, &fy, &mz);
      ASSERT_TRUE(IsEqual(fx, -5.2794538489462));
      ASSERT_TRUE(IsEqual(fy, -2.6327946288053));
      ASSERT_TRUE(IsEqual(mz, 18.616669354998));
      model.ComputePOIForces(lc, 204, MemberFaceType::Left, LoadOrientation::Member, &fx, &fy, &mz);
      ...
      pMember2.ReleaseEnd(MemberEndType::Start, MemberReleaseType::None);
