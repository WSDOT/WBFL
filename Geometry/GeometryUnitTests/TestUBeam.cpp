...
// TestUBeam.cpp
//
// Ported from F:\ARP\WBFL\Geometry\GeometryTest\TestUBeam.cpp (legacy
...
public:
   TEST_METHOD(TestIUBeam)
   {
         ...
         ASSERT_EQ(IsEqual(val,-30.75018),true);
   ...
   {
         CComPtr<IShape> shape;
         ASSERT_EQ( shape.CoCreateInstance( CLSID_UBeam ), S_OK );
         ...
         box->get_Left(&val);
         ASSERT_EQ( IsEqual(val,-42.715), true );

         box->get_Right(&val);
         ASSERT_EQ( IsEqual(val, 42.715), true );

         box->get_Top(&val);
         ASSERT_EQ( IsEqual(val, 60.00), true );
         ...
         ASSERT_EQ( IsEqual(area,1279.67674), true );
         ASSERT_EQ( IsEqual(ixx, 519561.213035), true );
         ASSERT_EQ( IsEqual(iyy, 999211.50319), true );
         ASSERT_EQ( IsEqual(ixy,0.0), true );
         cg->get_X(&cgx);
         cg->get_Y(&cgy);
         ASSERT_EQ( IsEqual(cgx,0.0), true );
         ASSERT_EQ( IsEqual(cgy,24.73653), true );
         ...
         ASSERT_EQ(IsEqual(xleft,42.715),true);
         ASSERT_EQ(IsEqual(xright,42.715),true);
         ASSERT_EQ(IsEqual(ytop,60.0-cgy),true);
         ...
         ASSERT_EQ( IsEqual(val,371.35986), true );
         ...
         ASSERT_EQ( IsEqual(val,85.43), true);
         ...
         ASSERT_EQ( IsEqual(val, 4.50), true);

         beamClone->get_D5(&val);
         ASSERT_EQ( IsEqual(val, 1.50), true);
         ...
         beamClone->get_T(&val);
         ASSERT_EQ( IsEqual(val, 7.00), true);
   ...
   {
         CComPtr<IXYPosition> position;
         ASSERT_EQ( position.CoCreateInstance( CLSID_UBeam), S_OK);
         ...
         hookPnt->get_X(&x);
         hookPnt->get_Y(&y);
         ASSERT_EQ(IsEqual(x,142.715), true );
         ...
         hookPnt->get_X(&x);
         hookPnt->get_Y(&y);
         ASSERT_EQ(IsEqual(x,100.0-42.715), true );
         ASSERT_EQ(IsEqual(y,100.0), true );
         ...
         hookPnt->get_Y(&y);
         ASSERT_EQ(IsEqual(x,100.0), true );
         ASSERT_EQ(IsEqual(y,70.9335), true );
         ...
         hookPnt->get_X(&x);
         hookPnt->get_Y(&y);
         ASSERT_EQ(IsEqual(x,57.285), true );
         ASSERT_EQ(IsEqual(y,70.0), true );
   ...
   TEST_METHOD(TestISupportErrorInfo)
   {
         ...
         ASSERT_EQ( eInfo.CoCreateInstance( CLSID_UBeam ), S_OK );
         ASSERT_EQ( eInfo != nullptr, true );

         // Interfaces that should be supported
         ASSERT_EQ( eInfo->InterfaceSupportsErrorInfo( IID_IUBeam ), S_OK );
