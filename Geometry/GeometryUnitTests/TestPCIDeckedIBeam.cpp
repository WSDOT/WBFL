...
// TestPCIDeckedIBeam.cpp
//
// Ported from F:\ARP\WBFL\Geometry\GeometryTest\TestPCIDeckedIBeam.cpp (legacy
...
{
public:
   TEST_METHOD(TestIPCIDeckedIBeam)
   {
         CComPtr<IPCIDeckedIBeam> beam;
         ASSERT_EQ( beam.CoCreateInstance(CLSID_PCIDeckedIBeam), S_OK );
         ...
         ASSERT_EQ(beam->get_D8(nullptr), E_POINTER);
         ASSERT_EQ(beam->get_D8(&val), S_OK);
         ASSERT_EQ(IsZero(val), true);

         ASSERT_EQ(beam->get_D9(nullptr), E_POINTER);
         ASSERT_EQ(beam->get_D9(&val), S_OK);
         ASSERT_EQ(IsZero(val), true);
   ...
   {
      	CComPtr<IShape> shape;
      	ASSERT_EQ(shape.CoCreateInstance(CLSID_PCIDeckedIBeam), S_OK);
         ...
         beam->put_C1(1.0);
      	...
      	box->get_Left(&val);
      	ASSERT_EQ(IsEqual(val, -58.5), true);
      	...
      	ASSERT_EQ(IsEqual(area, 941.66528114402706), true);
      	ASSERT_EQ(IsEqual(ixx, 420903.30302683858), true);
      	ASSERT_EQ(IsEqual(iyy, 403184.22410364990), true);
      	ASSERT_EQ(IsEqual(ixy, 0.0, 0.0005), true);
      	cg->get_X(&cgx);
      	cg->get_Y(&cgy);
      	ASSERT_EQ(IsEqual(cgx, 0.0), true);
      	ASSERT_EQ(IsEqual(cgy, 32.324778419863591), true);
      	...
      	ASSERT_EQ(IsEqual(xleft, 58.5), true);
      	ASSERT_EQ(IsEqual(xright, 58.5), true);
      	ASSERT_EQ(IsEqual(ytop, 54.0 - cgy), true);
      	...
      	ASSERT_EQ(IsEqual(val, 403.95659655723534), true);
   ...
   {
         CComPtr<IXYPosition> position;
         ASSERT_EQ( position.CoCreateInstance( CLSID_PCIDeckedIBeam), S_OK);
   ...
   TEST_METHOD(TestISupportErrorInfo)
   {
         ...
         ASSERT_EQ( eInfo.CoCreateInstance( CLSID_PCIDeckedIBeam ), S_OK );
         ASSERT_EQ( eInfo != nullptr, true );

         // Interfaces that should be supported
         ASSERT_EQ( eInfo->InterfaceSupportsErrorInfo( IID_IPCIDeckedIBeam ), S_OK );
