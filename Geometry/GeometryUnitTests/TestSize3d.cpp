...
// TestSize3d.cpp
//
// Ported from F:\ARP\WBFL\Geometry\GeometryTest\TestSize3d.cpp (legacy
...
TEST_CLASS(TestSize3d)
{
public:
   TEST_METHOD(BasicOperations)
   {
         CComPtr<ISize3d> pSize;
         ASSERT_EQ(pSize.CoCreateInstance(CLSID_Size3d),S_OK);
         ...
         ASSERT_EQ(pSize->put_Dz(dz),S_OK);
         ASSERT_EQ(pSize->get_Dz(nullptr),E_POINTER);
         ASSERT_EQ(pSize->get_Dz(&dz1),S_OK);
         ASSERT_EQ(IsEqual(dz,dz1),true);
         ...
         ASSERT_EQ(pSize->get_Dz(&dz1),S_OK);
         ASSERT_EQ(IsEqual(dz*scale,dz1),true);
   }

   TEST_METHOD(SupportsErrorInfo)
   {
         CComPtr<ISupportErrorInfo> eInfo;
         ASSERT_EQ( eInfo.CoCreateInstance( CLSID_Size3d ), S_OK );
         ASSERT_EQ( eInfo->InterfaceSupportsErrorInfo( IID_ISize3d ), S_OK );
