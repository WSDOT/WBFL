...
// TestPoint2dCollection.cpp
//
// Ported from F:\ARP\WBFL\Geometry\GeometryTest\TestPoint2dCollection.cpp (legacy
...
TEST_CLASS(TestPoint2dCollection)
{
public:
   TEST_METHOD(BasicOperations)
   {
         CComPtr<IPoint2dCollection> pColl;
         ASSERT_EQ(pColl.CoCreateInstance( CLSID_Point2dCollection ), S_OK);
   ...
   TEST_METHOD(SupportsErrorInfo)
   {
         ...
         ASSERT_EQ( eInfo.CoCreateInstance( CLSID_Point2dCollection ), S_OK );
         ASSERT_EQ( eInfo->InterfaceSupportsErrorInfo( IID_IPoint2dCollection ), S_OK );
