...
// TestVoidedSlab.cpp
//
// Ported from F:\ARP\WBFL\Geometry\GeometryTest\TestVoidedSlab.cpp (legacy
...
public:
   TEST_METHOD(TestIVoidedSlab)
   {
         ...
         ASSERT_EQ( beam.CoCreateInstance(CLSID_VoidedSlab), S_OK );
         ...
         ASSERT_EQ( beam->get_VoidSpacing(nullptr), E_POINTER );
         ASSERT_EQ( beam->get_VoidSpacing(&val), S_OK );
         ASSERT_EQ( IsZero(val), true );

         ASSERT_EQ( beam->get_VoidDiameter(nullptr), E_POINTER );
         ASSERT_EQ( beam->get_VoidDiameter(&val), S_OK );
         ASSERT_EQ( IsZero(val), true );
         ...
         ASSERT_EQ( beam->put_VoidSpacing(-1), GEOMETRY_E_DIMENSION );
         ASSERT_EQ( beam->put_VoidSpacing(12.5), S_OK );
         beam->get_VoidSpacing(&val);
         ASSERT_EQ( IsEqual(val,12.5), true );

         ASSERT_EQ( beam->put_VoidDiameter(-1), GEOMETRY_E_DIMENSION );
         ASSERT_EQ( beam->put_VoidDiameter(0.833), S_OK );
   ...
   {
         CComPtr<IShape> shape;
         ASSERT_EQ( shape.CoCreateInstance( CLSID_VoidedSlab ), S_OK );
         ...
         box->get_Left(&val);
         ASSERT_EQ( IsEqual(val,-24.0), true );
         ...
         ASSERT_EQ( IsEqual(area,862.365062561), true );
         ASSERT_EQ( IsEqual(ixx, 23327.929095931002), true );
         ASSERT_EQ( IsEqual(iyy, 165717.62311274296), true );
         ASSERT_EQ( IsEqual(ixy,0.0), true );
         cg->get_X(&cgx);
         cg->get_Y(&cgy);
         ASSERT_EQ( IsEqual(cgx,0.0), true );
         ASSERT_EQ( IsEqual(cgy,9.0), true );
         ...
         ASSERT_EQ(IsEqual(xleft,24.0),true);
         ASSERT_EQ(IsEqual(xright,24.0),true);
         ASSERT_EQ(IsEqual(ytop,9.0),true);
         ASSERT_EQ(IsEqual(ybottom,9.0),true);
         ...
         ASSERT_EQ( IsEqual(val,132.), true );
         ...
         i = 0;
         points[i++]->Location(&x, &y);
         ASSERT_EQ( IsEqual(x, 24.00), true );
         ...
         points[i++]->Location(&x, &y);
         ASSERT_EQ( IsEqual(x,-24.00), true );
         ASSERT_EQ( IsEqual(y, 18.00), true );
