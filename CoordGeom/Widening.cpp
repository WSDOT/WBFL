...
bool Widening::TestMe(WBFL::Debug::Log& rlog)
{
   TESTME_PROLOGUE("Widening");
   ...
   // before widening
   TRY_TESTME(IsEqual(widening->GetWidening(0.0, 0), 0.0));
   TRY_TESTME(IsEqual(widening->GetWidening(0.0, 1), 0.0));

   // half way through entry widening transition
   TRY_TESTME(IsEqual(widening->GetWidening(150.0, 0), 5.0));
   TRY_TESTME(IsEqual(widening->GetWidening(150.0, 1), 5.0));

   // full widening
   TRY_TESTME(IsEqual(widening->GetWidening(400.0, 0), 10.0));
   TRY_TESTME(IsEqual(widening->GetWidening(400.0, 1), 10.0));

   // half way through exit widening transition
   TRY_TESTME(IsEqual(widening->GetWidening(550.0, 0), 5.0));
   TRY_TESTME(IsEqual(widening->GetWidening(550.0, 1), 5.0));

   // after widening
   TRY_TESTME(IsEqual(widening->GetWidening(700.0, 0), 0.0));
   TRY_TESTME(IsEqual(widening->GetWidening(700.0, 1), 0.0));

   TESTME_EPILOG("Widening");
}
#endif // _UNITTEST
