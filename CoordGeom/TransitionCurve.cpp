...
#include <array>
bool Verify(Float64 L, const TransitionCurve& curve, std::array<std::pair<Float64, Float64>, 6>& values)
{
   bool bPassed = true;
   ...
   for (auto i = 0; i < size; i++)
   {
      ...
      auto p = curve.PointOnCurve(s);
      bPassed &= (p == WBFL::Geometry::Point2d(values[i].first, values[i].second));
   }
   return bPassed;
}

bool TransitionCurve::TestMe(WBFL::Debug::Log& rlog)
{
   TESTME_PROLOGUE("TransitionCurve");

   // CCW (left) curves
   WBFL::Geometry::Point2d startPnt(0, 0);
   Direction startDir(0);
   Float64 R1 = 0;
   Float64 R2 = 50;
   Float64 L = 100;
   TransitionCurve curve(startPnt, startDir, R1, R2, L, TransitionCurveType::Clothoid);

   std::array<std::pair<Float64, Float64>, 6> values1
   {
      std::make_pair(0.0, 0.0),
      std::make_pair(19.99680, 0.266636),
      std::make_pair(39.89772, 2.12944),
      std::make_pair(59.22705, 7.13362),
      std::make_pair(76.78475, 16.57380),
      std::make_pair(90.45228, 31.02682)
   };
   TRY_TESTME(Verify(L, curve, values1));

   R1 = 300;
   curve.Init(startPnt, startDir, R1, R2, L, TransitionCurveType::Clothoid);
   std::array<std::pair<Float64, Float64>, 6> values2
   {
      std::make_pair(0,0),
      std::make_pair(19.93117,1.55332),
      std::make_pair(39.46447,5.73933),
      std::make_pair(57.75644,13.71366),
      std::make_pair(73.27746,26.20778),
      std::make_pair(83.83594,43.06117)
   };
   TRY_TESTME(Verify(L, curve, values2));
   ...
   std::array<std::pair<Float64, Float64>, 6> values3
   {
      ...
      std::make_pair(39.89772, -2.12944),
      std::make_pair(59.22705, -7.13362),
      std::make_pair(76.78475, -16.57381),
      std::make_pair(90.45228, -31.02682)
   };
   TRY_TESTME(Verify(L, curve, values3));
   ...
   {
      std::make_pair(0, 0),
      std::make_pair(19.54638, -3.69183),
      std::make_pair(36.97649, -13.36558),
      std::make_pair(51.63105, -26.92687),
      std::make_pair(63.95106, -42.66644),
      std::make_pair(74.97974, -59.34911)
   };
   TRY_TESTME(Verify(L, curve, values4));

   TESTME_EPILOG("TransitionCurve");
}
#endif // _UNITTEST
