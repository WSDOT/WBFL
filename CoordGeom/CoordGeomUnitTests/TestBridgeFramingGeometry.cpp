...
std::shared_ptr<WBFL::COGO::BridgeFramingGeometry> CreateBridgeGeometry(const std::vector<std::tuple<IDType, Float64, std::_tstring, ConnectionGeometry, ConnectionGeometry>>& piers, bool bContinuousGirders)
{
   return CreateBridgeGeometry(0.00, 10.0, 10.0, M_PI / 4,piers,bContinuousGirders);
}

std::shared_ptr<WBFL::COGO::BridgeFramingGeometry> CreateBridgeGeometry(Float64 refStation, Float64 x, Float64 y, Float64 direction,const std::vector<std::tuple<IDType, Float64, std::_tstring, ConnectionGeometry, ConnectionGeometry>>& piers, bool bContinuousGirders)
{
   // create the alignment
   auto alignment = CreateStraightAlignment(refStation, x,y,direction);
...
namespace CoordGeomUnitTest
{
	...
	public:
      ...
      TEST_METHOD(Test1)
      {
         ...
         Assert::IsTrue(bridge->GetGirderLineCount() == 5);

         auto pier1 = bridge->FindPierLine(100);
         auto pier2 = bridge->FindPierLine(200);

         Assert::IsTrue(pier1->GetAlignmentPoint() == WBFL::Geometry::Point2d(80.710678, 80.710678));
         Assert::IsTrue(pier2->GetAlignmentPoint() == WBFL::Geometry::Point2d(151.421356, 151.421356));

         Assert::IsTrue(pier1->GetDirection() == Direction(3 * M_PI / 4));
         Assert::IsTrue(pier2->GetDirection() == Direction(3 * M_PI / 4));

         auto girder_line = bridge->FindGirderLine(502);

         Assert::IsTrue(IsEqual(girder_line->GetLayoutLength(), 100.0));
         Assert::IsTrue(girder_line->GetPierPoint(EndType::Start) == WBFL::Geometry::Point2d(80.710678, 80.710678));
         Assert::IsTrue(girder_line->GetPierPoint(EndType::End) == WBFL::Geometry::Point2d(151.421356, 151.421356));

         Assert::IsTrue(girder_line->GetBearingPoint(EndType::Start) == WBFL::Geometry::Point2d(80.710678 + 2.5 * cos(M_PI / 4), 80.710678 + 2.5 * sin(M_PI / 4)));
         Assert::IsTrue(girder_line->GetBearingPoint(EndType::End) == WBFL::Geometry::Point2d(151.421356 - 2.5 * cos(M_PI / 4), 151.421356 - 2.5 * cos(M_PI / 4)));

         Assert::IsTrue(girder_line->GetEndPoint(EndType::Start) == WBFL::Geometry::Point2d(80.710678 + 1.5 * cos(M_PI / 4), 80.710678 + 1.5 * sin(M_PI / 4)));
         Assert::IsTrue(girder_line->GetEndPoint(EndType::End) == WBFL::Geometry::Point2d(151.421356 - 1.5 * cos(M_PI / 4), 151.421356 - 1.5 * cos(M_PI / 4)));
      }
      ...
      TEST_METHOD(Test2)
      {
         ...
         Assert::IsTrue(bridge->GetGirderLineCount() == 10);
      }
      ...
      TEST_METHOD(Test3)
      {
         ...
         Assert::IsTrue(bridge->GetGirderLineCount() == 9);
      }
      ...
      TEST_METHOD(Test4)
      {
         ...
         Assert::IsTrue(nDiaphragmLines == 5); // 3 in span 1 and 1 in spans 2 and 3
         for (IDType gdrID = 500; gdrID < 504; gdrID++)
         {
            for (IndexType i = 0; i < 3; i++) // there are only 3 in span 1
            {
               ...
               Assert::IsTrue(diaphragm_line->GetID() == (IDType)i);

               auto length = diaphragm_line->Length(gdrID);
               Assert::IsTrue(IsEqual(length, 5.0));
               ...
               Assert::IsTrue(start_point == expected_start_point);
               Assert::IsTrue(end_point == expected_end_point);
            }
         }
      ...
      TEST_METHOD(Test5)
      {
         ...
         for (const auto& point : points)
         {
            ...
            Assert::IsTrue(point == expected_point);
