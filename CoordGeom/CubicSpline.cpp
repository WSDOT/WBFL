...
bool CubicSpline::TestMe(WBFL::Debug::Log& rlog)
{
   TESTME_PROLOGUE("CubicSpline");
   ...
   TRY_TESTME(p == WBFL::Geometry::Point2d(-0.98058067569092022, 1.1961161351381839));
   p = spline.PointOnCurve(0);
   TRY_TESTME(p == WBFL::Geometry::Point2d(0, 1.0));

   // test spline slope (derivative = bearing)
   auto d = spline.GetBearing(-1);
   TRY_TESTME(IsEqual(tan(d), -0.2));

   d = spline.GetBearing(0);
   TRY_TESTME(IsEqual(tan(d), -0.2));

   d = spline.GetBearing(50);
   TRY_TESTME(IsEqual(tan(d), -2.5));

   // spline length
   TRY_TESTME(IsEqual(spline.GetLength(), 7.8778168718814285));

   // check the normal
   TRY_TESTME(IsEqual(1 / 0.2, tan(spline.GetNormal(0))));
   TRY_TESTME(IsEqual(1 / 0.2, tan(spline.GetNormal(-10))));

   // bearing and normal check at end
   TRY_TESTME(IsEqual(-2.5, tan(spline.GetBearing(spline.GetLength()))));
   TRY_TESTME(IsEqual(-2.5, tan(spline.GetBearing(spline.GetLength()+10))));

   TRY_TESTME(IsEqual(1 / 2.5, tan(spline.GetNormal(spline.GetLength()))));
   TRY_TESTME(IsEqual(1 / 2.5, tan(spline.GetNormal(spline.GetLength() + 10))));
   ...
   TRY_TESTME(pntOnSpline == WBFL::Geometry::Point2d(2.9212265014648437, 0.90555618284927675));
   TRY_TESTME(IsEqual(distFromStart, 3.8186530950157791));
   TRY_TESTME(bOnProjection == false);

   // project a point before start of spline
   p.Move(-10, 0);
   std::tie(pntOnSpline, distFromStart, bOnProjection) = spline.ProjectPoint(p);
   TRY_TESTME(pntOnSpline == WBFL::Geometry::Point2d(-9.4230769230769234, 2.8846153846153855));
   TRY_TESTME(IsEqual(distFromStart, -9.6096906217710174));
   TRY_TESTME(bOnProjection == true);

   // project a point after start of spline
   p.Move(10, 0);
   std::tie(pntOnSpline, distFromStart, bOnProjection) = spline.ProjectPoint(p);
   TRY_TESTME(pntOnSpline == WBFL::Geometry::Point2d(6.8965517241379324, -1.2413793103448272));
   TRY_TESTME(IsEqual(distFromStart, 10.291856268183103));
   TRY_TESTME(bOnProjection == true);
   ...
   TRY_TESTME(vPoints.size() == 3);
   TRY_TESTME(vPoints[0] == WBFL::Geometry::Point2d(0, 1));
   TRY_TESTME(vPoints[1] == WBFL::Geometry::Point2d(3, 1));
   TRY_TESTME(vPoints[2] == WBFL::Geometry::Point2d(6, 1));
   ...
   TRY_TESTME(vPoints.size() == 3);
   TRY_TESTME(vPoints[0] == WBFL::Geometry::Point2d(0.56655, 0.5));
   TRY_TESTME(vPoints[1] == WBFL::Geometry::Point2d(2.56441, 0.5));
   TRY_TESTME(vPoints[2] == WBFL::Geometry::Point2d(6.19999, 0.5));
   ...
   TRY_TESTME(vPoints.size() == 3);
   TRY_TESTME(vPoints[0] == WBFL::Geometry::Point2d(0, 1));
   TRY_TESTME(vPoints[1] == WBFL::Geometry::Point2d(4, 2));
   TRY_TESTME(vPoints[2] == WBFL::Geometry::Point2d(4.39108, 2.09777));
   ...
   TRY_TESTME(IsEqual(spline.GetLength(), (2 * M_PI * R) / 4, 0.01));
   ...
   TRY_TESTME(p == WBFL::Geometry::Point2d(2, 0));
   ...
   p = spline.PointOnCurve(0.);
   TRY_TESTME(p == WBFL::Geometry::Point2d(0, 0));

   p = spline.PointOnCurve(2.);
   TRY_TESTME(p == WBFL::Geometry::Point2d(1.98136, 0.23124));

   p = spline.PointOnCurve(spline.GetLength());
   TRY_TESTME(p == WBFL::Geometry::Point2d(10, 10));
   ...
   p = spline.PointOnCurve(0);
   TRY_TESTME(p == WBFL::Geometry::Point2d(25.950838, 47.9880235));

   p = spline.PointOnCurve(spline.GetLength());
   TRY_TESTME(p == WBFL::Geometry::Point2d(148.83656, 196.75139));

   TESTME_EPILOG("CubicSpline");
}
#endif // _UNITTEST
