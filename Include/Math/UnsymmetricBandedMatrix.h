...
/// 
/// [A]{B} = {C} [Matrix A]*}{Vector B} = {Vector C}
...
///       [0|7|6|3]
///       [0|0|4|9]
class MATHCLASS mathUnsymmetricBandedMatrix
{
public:
   enum class Storage
   {
   ...
   mathUnsymmetricBandedMatrix() = default;
   mathUnsymmetricBandedMatrix(IndexType N, IndexType BW,Storage storage = Storage::Column);

   // Creates the matrix and vector. The matrix object takes ownership of ba and b
   mathUnsymmetricBandedMatrix(IndexType N, IndexType BW,Storage storage, Float64** ba, Float64* b);
   ~mathUnsymmetricBandedMatrix();

   // copy semantics are non-trivial and have not been implemented
   mathUnsymmetricBandedMatrix(const mathUnsymmetricBandedMatrix&) = delete;
   mathUnsymmetricBandedMatrix& operator=(const mathUnsymmetricBandedMatrix&) = delete;
