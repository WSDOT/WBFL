...
namespace WBFL
{
   namespace RCSection
   {
      class AxialInteractionCurveSolverImpl;

      /// Computes a moment interaction diagram for a GeneralSection object.
      /// This solver computes P-M values for a given neutral axis angle
      class RCSCLASS AxialInteractionCurveSolver
      {
      public:
         AxialInteractionCurveSolver();
         AxialInteractionCurveSolver(const AxialInteractionCurveSolver& other) = delete; // can't copy because of unique_ptr - also don't need copy semantics
         ~AxialInteractionCurveSolver();
         
         AxialInteractionCurveSolver& operator=(const AxialInteractionCurveSolver& other) = delete; // can't assign
         ...
         std::unique_ptr<InteractionCurveSolution> Solve(
            Float64 na, ///< direction of the neutral axis (radians) measured counter-clockwise from X-axis
            IndexType nFzSteps ///< Number of axial load steps between the compression and tension limits
            ) const;

      private:
         std::unique_ptr<AxialInteractionCurveSolverImpl> m_pImpl;
      };
   };
};