...
namespace WBFL
{
   namespace EAF
   {
      class EAFCLASS PluginCommandManager  
      {
      public:
         PluginCommandManager() = default;
         virtual ~PluginCommandManager() = default;
         ...
         // Adds a new command and the object to call when it is executed. Returns a unique
         ...
         std::vector<UINT> GetMappedCommandIDs(std::shared_ptr<WBFL::EAF::ICommandCallback>& pCallback);

         // Given an original command ID and a callback, retrieves the mapped (unique) command id
         std::pair<BOOL,UINT> GetMappedCommandID(UINT nPluginCmdID, std::shared_ptr<WBFL::EAF::ICommandCallback>& pCallback);
      ...
      private:
         ...
         UINT m_nBaseID = INVALID_ID; // ID of first unique mapped id
         UINT m_nCommands = INVALID_ID; // number of commands reserved for this command manager
         UINT m_nNextID = INVALID_ID;
