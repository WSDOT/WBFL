...
namespace WBFL
{
   namespace EAF
   {
      ...
      class EAFCLASS TxnManagerFactory
      {
      public:
         virtual std::unique_ptr<TxnManager> CreateTransactionManager() = 0;
      };
      ...
      /// is a global resource object and the EAF framework gains access to it by calling TxnManager::GetInstance().
      /// TxnManager::SetTransactionManagerFactory() must be called prior to the first call to TxnManager::GetInstance()
      /// for your custom transaction manager to be instantiated.
      /// 
      /// CDocument-derived classes must call TxnManager::ShutDown() in an overwritten CDocument::DeleteContents() method.
      class EAFCLASS TxnManager
      {
      public:
         TxnManager() = default;
         ...
         static void SetTransactionManagerFactory(std::unique_ptr<TxnManagerFactory>&& pFactory);

         /// @brief Returns the only instance of the transaction manager
         static TxnManager& GetInstance();

         /// @brief The document class should call this method in CDocument::DeleteContents
         ...
         virtual void Execute(const Transaction& rTxn);

         /// @brief Executes a transaction.
         /// The transaction is maintained in the transaction history
         /// and will be used for Undo/Redo/Repeat if permitted
         virtual void Execute(std::unique_ptr<Transaction>&& pTxn);
      ...
      protected:
         using TxnContainer = std::list<std::unique_ptr<Transaction>>;

         TxnContainer m_TxnHistory;   // Txn that have been executed
         TxnContainer m_UndoHistory;  // Txn that have been undone

         static std::unique_ptr<TxnManagerFactory> ms_pFactory;
         static std::unique_ptr<TxnManager> ms_pInstance;
