...
// TestPoiPlacement.cpp: implementation of the CTestPoiPlacement class.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "TestPoiPlacement.h"
#include <fstream>
...
CTestPoiPlacement::CTestPoiPlacement()
{
...
HRESULT CTestPoiPlacement::Test()
{
   ...
   HRESULT hr = plgr.CoCreateInstance(CLSID_LoadGroupForceResponse);
...
//   plgr.CoCreateInstance(CLSID_LoadGroupDeflectionResponse);
   ...
   // test persistence by saving and retrieving data and comparing raw xml
   {
      ...
      TRY_TEST( pss->Open( CComBSTR("TestPoiPlacement.xml")), S_OK );
...
ILBAMModel* CTestPoiPlacement::CreateModel()
{
   CComPtr<ILBAMModel> psm;
...
//===========================
   ...
   Float64 loc;
/*    We only generate pois at locations where we expect poi maps to be of the spanning type in the model
...
*/
   ...
   // span 0
   {
      CComPtr<IPOI> pPOI;
      TRY_TEST(pPOI.CoCreateInstance( CLSID_POI ), S_OK );
      TRY_TEST( pPOI->put_ID(200), S_OK );
      TRY_TEST( pPOI->put_MemberType(mtSpan), S_OK );
      TRY_TEST( pPOI->put_MemberID(0), S_OK );
      TRY_TEST( pPOI->put_Location(-0.8), S_OK );
      TRY_TEST(pPOIs->Add(pPOI), S_OK);
   }
   ...
   for (MemberIDType i=0; i<=2; i++)
   {
      ...
      TRY_TEST( pPOI1->put_Location(-0.5), S_OK );
      TRY_TEST(pPOIs->Add(pPOI1), S_OK);

      poi_id += 1;
      TRY_TEST(pPOI2.CoCreateInstance( CLSID_POI ), S_OK );
      TRY_TEST( pPOI2->put_ID(poi_id), S_OK );
      TRY_TEST( pPOI2->put_MemberType(mtSuperstructureMember), S_OK );
      TRY_TEST( pPOI2->put_MemberID(i), S_OK );
      TRY_TEST( pPOI2->put_Location(-1.0), S_OK );
