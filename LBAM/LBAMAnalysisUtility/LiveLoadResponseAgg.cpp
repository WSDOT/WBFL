...
STDMETHODIMP CLiveLoadResponseAgg::InterfaceSupportsErrorInfo(REFIID riid)
{
                                                 ...
                                                 VARIANT_BOOL computePlacement, ILiveLoadModelSectionResults** pResults)
{
   ...
   try
   {
      ...
      for (CollectionIndexType ieng=0;  ieng<eng_cnt; ieng++)
      {
         ...
         hr = engine->get_LiveLoadModelResponse(&response);

         CComPtr<ILiveLoadModelSectionResults> results;

         hr = response->ComputeForces(pois,stage,type,orientation,effect,optimization,
                                      vehConfiguration,applyImpact,applyDistribution,
                                                      ...
                                                      VARIANT_BOOL computePlacement, ILiveLoadModelSectionResults** pResults)
{
   ...
   try
   {
      ...
      for (CollectionIndexType ieng=0;  ieng<eng_cnt; ieng++)
      {
         ...
         hr = response->ComputeDeflections(pois,stage,type,effect,env_optimization,
                                           vehConfiguration,applyImpact,applyDistribution,
                                           computePlacement,&results);
      ...
      for (CollectionIndexType ieng=1; ieng<eng_cnt; ieng++)
      {
         EnvelopeLiveLoadSectionResults(result_vec[0].m_T,result_vec[ieng].m_T,computePlacement,optimization,false,ieng);
      }
...
STDMETHODIMP CLiveLoadResponseAgg::ComputeReactions(IIDArray* supports, BSTR stage, LiveLoadModelType type, 
                                                    ...
                                                    VehicularLoadConfigurationType vehConfiguration, 
                                                    VARIANT_BOOL applyImpact, VARIANT_BOOL applyDistribution,
                                                    VARIANT_BOOL computePlacement, ILiveLoadModelResults** pResults)
{
   CHECK_RETOBJ(pResults);
   CHRException hr;
   ...
   try
   {
      ...
      for (CollectionIndexType ieng=0;  ieng<eng_cnt; ieng++)
      {
         ...
         hr = response->ComputeReactions(supports,stage,type,effect,env_optimization,
                                         vehConfiguration,applyImpact,applyDistribution,
                                         computePlacement,&results);
                                                             ...
                                                             VARIANT_BOOL computePlacement, ILiveLoadModelResults** pResults)
{
   ...
   try
   {
      ...
      for (CollectionIndexType ieng=0;  ieng<eng_cnt; ieng++)
      {
         ...
         hr = response->ComputeSupportDeflections(supports,stage,type,effect,env_optimization,
                                                  vehConfiguration,applyImpact,applyDistribution,
                                                  computePlacement,&results);
...
STDMETHODIMP CLiveLoadResponseAgg::ComputeStresses(IIDArray* pois, BSTR stage, LiveLoadModelType type, 
                                                   ...
                                                   VehicularLoadConfigurationType vehConfiguration, 
                                                   VARIANT_BOOL applyImpact, VARIANT_BOOL applyDistribution,
                                                   VARIANT_BOOL computePlacement, ILiveLoadModelStressResults** pResults)
{
   CHECK_RETOBJ(pResults);
   CHRException hr;
   ...
   try
   {
      ...
      hr = ComputeForces(pois,stage,type,roMember,effect,optimization,
                         vehConfiguration,applyImpact,applyDistribution,
...
void CLiveLoadResponseAgg::EnvelopeLiveLoadResults(ILiveLoadModelResults* res1, ILiveLoadModelResults* res2,
