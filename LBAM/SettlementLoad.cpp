...
STDMETHODIMP CSettlementLoad::Clone(ISettlementLoad **clone)
{
   CHECK_RETOBJ(clone);
   ...
   CComPtr<ISettlementLoad> pscs(pnew); // for reference count
...
STDMETHODIMP CSettlementLoad::Load(IStructuredLoad2 * pload)
{
   ...
   {        
      ...
      var.Clear();
      hr = pload->get_Property(_bstr_t("Dx"),&var);
      if (FAILED(hr))
         return hr;
      ...
      var.Clear();
      hr = pload->get_Property(_bstr_t("Dy"),&var);
      if (FAILED(hr))
         return hr;
      ...
      var.Clear();
      hr = pload->get_Property(_bstr_t("Rz"),&var);
      if (FAILED(hr))
         return hr;
...
STDMETHODIMP CSettlementLoad::Save(IStructuredSave2 * psave)
{
   ...
   try
   {
...

      {
         ...
         hr = psave->put_Property(CComBSTR("Dx"),_variant_t(m_Dx));
         hr = psave->put_Property(CComBSTR("Dy"),_variant_t(m_Dy));
         hr = psave->put_Property(CComBSTR("Rz"),_variant_t(m_Rz));
      }
   ...
   {
      return DealWithExceptions(this, IID_ISettlementLoad);
   }
