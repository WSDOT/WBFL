...
STDMETHODIMP CAxle::Load(IStructuredLoad2 * pload)
{
   ...
   {        
      _variant_t var;
      hr = pload->get_Property(_bstr_t("Weight"),&var);
      if (FAILED(hr))
         return hr;
      ...
      hr = pload->get_Property(_bstr_t("Spacing"),&var);
      if (FAILED(hr))
         return hr;
...
STDMETHODIMP CAxle::Save(IStructuredSave2 * psave)
{
   ...
   hr = psave->put_Property(CComBSTR("Weight"),_variant_t(m_Weight));
   if (FAILED(hr))
      return hr;

   hr = psave->put_Property(CComBSTR("Spacing"),_variant_t(m_Spacing));
   if (FAILED(hr))
      return hr;
...
STDMETHODIMP CAxle::Clone(IAxle **clone)
{
   CHECK_RETOBJ(clone);
