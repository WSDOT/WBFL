...
// TestTemporarySupport.cpp: implementation of the CTestTemporarySupport class.
//
//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
CTestTemporarySupport::CTestTemporarySupport():
...
void CTestTemporarySupport::Test()
{
   HRESULT hr;

   // create a listener
   CComObject<CTestTemporarySupport>* pMe;
   CComObject<CTestTemporarySupport>::CreateInstance(&pMe);
   pMe->AddRef();

   // first test a sm by itself
   CComPtr<ITemporarySupport> psm;
   TRY_TEST(psm.CoCreateInstance( CLSID_TemporarySupport ), S_OK );
   Float64 d;
   TRY_TEST( psm->get_Length(&d), S_OK );
   TRY_TEST( d, 0.0 );
   TRY_TEST( psm->get_BottomOffset(&d), S_OK );
   TRY_TEST( d, 0.0 );
   TRY_TEST( psm->get_Location(&d), S_OK );
   TRY_TEST( d, 0.0 );
   ...
   TRY_TEST( stgn, CComBSTR("") );

   // let's listen to pcs
   DWORD dwCookie;
   CComPtr<IUnknown> punk(pMe);
   TRY_TEST(AtlAdvise(psm,punk,IID_ITemporarySupportEvents,&dwCookie),S_OK);
   ...
   TRY_TEST( psm->get_BottomOffset(&d), S_OK );
   TRY_TEST( d, 1.3 );
   TRY_TEST( psm->put_Location(3.2), S_OK );
   TEST_STIFFNESS_FIRED();
   TRY_TEST( psm->get_Location(&d), S_OK );
   TRY_TEST( d, 3.2 );
   ...
   TRY_TEST( psm->put_Length(-1), E_INVALIDARG );

   TRY_TEST( psm->put_StageRemoved(CComBSTR("stage 2")), S_OK );
   TEST_STIFFNESS_FIRED();
   stgn.Empty();
   TRY_TEST( psm->get_StageRemoved(&stgn), S_OK );
...

   {
      ...
      CComPtr<ITemporarySupport> psm2;
      TRY_TEST(psm2.CoCreateInstance( CLSID_TemporarySupport ), S_OK );
   ...
   TRY_TEST(AtlUnadvise(psm,IID_ITemporarySupportEvents,dwCookie),S_OK);
   pMe->Release();
}



HRESULT CTestTemporarySupport::OnTemporarySupportChanged(ITemporarySupport* TemporarySupport, BSTR stage,ChangeType change)
{
