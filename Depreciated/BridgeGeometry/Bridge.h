...
public:
	CBridge()
	{
      m_BridgeAlignmentID = INVALID_ID;
      m_ProfileID = INVALID_ID;
      m_SurfaceID = INVALID_ID;
...
END_COM_MAP()
	...
	HRESULT FinalConstruct()
	{
      ...
      CComObject<CPathCollection>* pLayoutLines;
      hr = CComObject<CPathCollection>::CreateInstance(&pLayoutLines);
      if ( FAILED(hr) )
         return hr;

      m_LayoutLines = pLayoutLines;
   ...
   using AlignmentCollection = std::map<CogoObjectID,CComPtr<IAlignment>>;
   AlignmentCollection m_Alignments;

   CogoObjectID m_BridgeAlignmentID; // ID of the primary bridge alignment

   CogoObjectID m_ProfileID; // ID of the profile for the bridge alignment
   CogoObjectID m_SurfaceID; // ID of the finished surface for the roadway
   ...
   CComPtr<IPath> m_BridgeLine; // this is the bridge reference line. it is a path that is parallel to the bridge alignment
   ...
   using GirderLineCollection = std::map<GirderIDType, CComPtr<IGirderLine>>;
   using GirderLineEntry = std::pair<GirderIDType,CComPtr<IGirderLine>>;
   GirderLineCollection m_GirderLines;
   
   using DiaphragmLineCollection = std::map<GirderIDType, CComPtr<IDiaphragmLine>>;
   using DiaphragmLineEntry = std::pair<GirderIDType,CComPtr<IDiaphragmLine>>;
