...
CollectionIndexType rcaCapacityProblem::AddConcreteElement(const WBFL::Geometry::Shape& shape, CollectionIndexType concreteKey, Float64 nFactor,bool bIsVoid)
{
   OnGeometryChange();

   ConcElementPtr pelm(new rcaConcreteElement(shape, concreteKey, nFactor, bIsVoid, *this));
   CollectionIndexType key = GetNextAvailableKey();
...
CollectionIndexType rcaCapacityProblem::AddConcreteElement(const WBFL::Geometry::Shape& shape, CollectionIndexType concreteKey,bool bIsVoid)
{
   OnGeometryChange();

   ConcElementPtr pelm(new rcaConcreteElement(shape, concreteKey, bIsVoid, *this));
   CollectionIndexType key = GetNextAvailableKey();
   ConcElementEntry entry(key,pelm);
   std::pair<ConcElementContainer::iterator,bool> retval = m_ConcElementContainer.insert( entry );
   return key;
...
CollectionIndexType rcaCapacityProblem::AddReinforcementElement(const WBFL::Geometry::Shape& shape, CollectionIndexType matId,
                                                    Float64 initStrain)
{
   OnGeometryChange();
...
void rcaCapacityProblem::SetNeutralAxisLocation(const WBFL::Geometry::Line2d& line)
{
   m_NeutralAxisLocation = line;
   OnNeutralAxisChanged();
}

void rcaCapacityProblem::GetNeutralAxisLocation(WBFL::Geometry::Line2d* line) const
{
...
void rcaCapacityProblem::GetCompressionBlockBoundary(WBFL::Geometry::Line2d* line) const
{
   *line = m_NeutralAxisLocation.Parallel(m_CbOffset, WBFL::Geometry::Line2d::Side::Left);
}

Float64 rcaCapacityProblem::GetStrain( const WBFL::Geometry::Point2d& p ) const
{
...
Float64 rcaCapacityProblem::GetStrain( CollectionIndexType id ) const
{
   ...
   if ( riter != m_ReinfElementContainer.end() )
   {
      ...
      props = rept->GetShape().GetProperties();
      found = true;
   }
   ...
   if (!found)
   {
      ...
      if ( citer != m_ConcElementContainer.end() )
      {
         ...
         props = cept->GetShape().GetProperties();
         found = true;
      }
   }

   if (found)
      return GetStrain(props.GetCentroid());
...
void rcaCapacityProblem::GetSectionForcesEx(Float64* pFz,
                                            Float64* pMx,
                                            Float64* pMy,
                                            Float64* pcompRes, WBFL::Geometry::Point2d* pcompLoc,
                                            Float64* ptensRes, WBFL::Geometry::Point2d* ptensLoc) const
{
   WBFL::Geometry::Point2d pc = GetPlasticCentroid();
   Float64 gfz, gmx, gmy;
...
void rcaCapacityProblem::GetSectionForcesEx(rcaSectionForceDetails* pDetails) const
{
        ...
        riter++)
   {
      ...
      const WBFL::Geometry::Shape& shape = rept->GetShape();
      WBFL::Geometry::ShapeProperties props = shape.GetProperties();
      rfd.Ab = props.GetArea();
      rfd.LocBar = props.GetCentroid();
                                          ...
                                          Float64* pMy) const
{
   ...
   WBFL::Geometry::Point2d compLoc, tensLoc;
...
void rcaCapacityProblem::GetTensileCapacity(Float64* pPt,Float64* pMxt,Float64* pMyt) const
{
   ...
   WBFL::Geometry::Point2d pc = GetPlasticCentroid(); // This is the plastic centroid for
         ...
         iter++ )
   {
      ...
      const WBFL::Geometry::Shape& shape = rept->GetShape();
      WBFL::Geometry::ShapeProperties prop = shape.GetProperties();
      Float64 Aps = prop.GetArea();
      WBFL::Geometry::Point2d cg = prop.GetCentroid();
...
void rcaCapacityProblem::GetCompressiveCapacity(Float64* pPc,Float64* pMxc,Float64* pMyc) const
{
   ...
   WBFL::EngTools::BiaxialBeamStrain save_bs = m_BeamStrain;
...
WBFL::Geometry::Point2d rcaCapacityProblem::GetPlasticCentroid() const
{
   ...
   if (m_PcHasChanged)
   {
      ...
      WBFL::Geometry::Rect2d box = this->GetBoundingBox(); 
...
WBFL::Geometry::Rect2d rcaCapacityProblem::GetBoundingBox() const
{
   // cycle through all concrete, rebar and strands to determine the bounding box
   WBFL::Geometry::Rect2d sum_box;
   bool first=false;
...
void rcaCapacityProblem::Dump(dbgDumpContext& os) const
{
   ...
   if ( m_ReinfElementContainer.size() != 0 )
   {
      ...
      for ( reinf_iter = m_ReinfElementContainer.begin(); reinf_iter != m_ReinfElementContainer.end(); reinf_iter++ )
      {
         ...
         WBFL::Geometry::ShapeProperties props = pElement->GetShape().GetProperties();

         os << pElement->GetInitialStrain() << "," 
            << props.GetCentroid().X() << "," << props.GetCentroid().Y() << endl;
      }
   }
}
#endif // _DEBUG
...
void rcaCapacityProblem::OnNeutralAxisChanged()
{
   ...
   m_CompArea = GetClippedConcreteArea(m_NeutralAxisLocation,WBFL::Geometry::Line2d::Side::Right);

   // determine effective area of compression block
   WBFL::Geometry::Line2d cbLine = m_NeutralAxisLocation.Parallel(m_CbOffset,WBFL::Geometry::Line2d::Side::Left);
   m_EffCompArea = GetClippedConcreteArea(cbLine,WBFL::Geometry::Line2d::Side::Right);
}
...
Float64 rcaCapacityProblem::DoCalculateFurthestDistance()
{
         ...
         iter++ )
   {
      ...
      const WBFL::Geometry::Shape& shape = ptr->GetShape();
      Float64 current_dist = shape.GetFurthestDistance( m_NeutralAxisLocation, WBFL::Geometry::Line2d::Side::Left );
...
Float64 rcaCapacityProblem::GetClippedConcreteArea(const WBFL::Geometry::Line2d& clipLine, WBFL::Geometry::Line2d::Side side) const
{
   Float64 area = 0;
   for (ConstConcElementIterator citer = m_ConcElementContainer.begin(); 
        citer!=m_ConcElementContainer.end(); citer++)
...
Float64 rcaCapacityProblem::GetClippedConcreteArea(CollectionIndexType concreteIdx, const WBFL::Geometry::Line2d& clipLine, WBFL::Geometry::Line2d::Side side) const
{
   CHECK(0 <= concreteIdx && concreteIdx < m_ConcreteContainer.size());
...
void rcaCapacityProblem::GetGlobalSectionForcesEx(Float64* pFz,
                                                  Float64* pMx,
                                                  Float64* pMy,
                                                  Float64* compRes, WBFL::Geometry::Point2d* compLoc,
                                                  Float64* tensRes, WBFL::Geometry::Point2d* tensLoc) const
{
   ...
   *tensRes = tensfz;
   if (!IsZero(tensfz))  tensLoc->Move(-tensmy/tensfz, tensmx/tensfz);

   *compRes = compfz;
   if ( !IsZero(compfz)) compLoc->Move(-compmy/compfz, compmx/compfz);
}
...
//======================== INQUERY    =======================================
#if defined _DEBUG
dbgDumpContext& operator<<(dbgDumpContext& os,const WBFL::Geometry::Point2d& p)
{
   os << _T("(") << p.X() << _T(",") << p.Y() << _T(")");
   return os;
}
...
void rcaSectionForceDetails::Dump(dbgDumpContext& os) const
{
   ...
   os << _T("NeutralAxis (O)  = ") << (NeutralAxis.PointOnLineNearest(WBFL::Geometry::Point2d(0,0)).Y()) << endl;
