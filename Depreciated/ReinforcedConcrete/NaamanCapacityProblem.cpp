...
bool rcaNaamanCapacityProblem::TestMe(dbgLog& rlog)
{
   ...
   //   (-10,0) = Hook Point
   //
   WBFL::Geometry::Triangle shape(WBFL::Geometry::Point2d(-10,0),50,20,10);
   prob.AddConcreteElement(shape,conc_idx,1.0);

   WBFL::Geometry::Circle circle(WBFL::Geometry::Point2d(0,5),1);
   prob.AddReinforcementElement(circle,rebar_idx,0.0);

   WBFL::Geometry::Line2d na( WBFL::Geometry::Point2d(-10,25), WBFL::Geometry::Point2d(10,25) );
...
Float64 rcaNaamanCapacityProblem::Evaluate(Float64 x) const
{
   ...
   Float64 cla;
   WBFL::Geometry::Line2d cbb = m_NaLine.Parallel(x,WBFL::Geometry::Line2d::Side::Left);

   // get the clipped concrete area - this is factored by N
   cla = GetClippedConcreteArea(cbb, WBFL::Geometry::Line2d::Side::Right);
