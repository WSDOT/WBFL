...
void CTestFrameWithReleases::Test2()
{
   ...
   TRY_TEST_MC(pJoints->Create(2, 10.0,   0.0, &pJoint2));
   ...
   TRY_TEST_HR(pPOIs->Create(200,  1,  0.00,  &pPOI200));
   TRY_TEST_HR(pPOIs->Create(201,  1, -0.25,  &pPOI201));
   TRY_TEST_HR(pPOIs->Create(202,  1, -0.50,  &pPOI202));
   TRY_TEST_HR(pPOIs->Create(203,  1, -0.75,  &pPOI203));
   TRY_TEST_HR(pPOIs->Create(204,  1, -1.00,  &pPOI204));
   ...
   // load case 0
   // vertical point load at the center of member 1
   ...
   TRY_TEST_LC(pPointLoads->Create(1, 1, -0.5, 0.0, -10.0, 0.0, lotGlobal, &pPointLoad));

   // load case 1
   // uniform axial strain along member 1
   ...
   TRY_TEST_LC(pMemberStrains->Create(1, 1, 0.0, -1.0, 2, 0, &pStrainLoad));

   pMemberStrains.Release();
   pStrainLoad.Release();

   // load case 2
   // uniform curvature along member 1
   ...
   TRY_TEST_LC(pMemberStrains->Create(1, 1, 0.0, -1.0, 0, 1, &pStrainLoad));
   ...
   TRY_TEST_HR(presults->ComputeJointDeflections(lc, 1, &dx, &dy, &rz));
   TRY_TEST_B( IsEqual(dx,  0.0) );
   TRY_TEST_B( IsEqual(dy, 50.0) );
   TRY_TEST_B( IsEqual(rz,-10.0) );
   TRY_TEST_HR(presults->ComputeJointDeflections(lc, 2, &dx, &dy, &rz));
   TRY_TEST_B( IsEqual(dx,  0.0) );
   TRY_TEST_B( IsEqual(dy,  0.0) );
   TRY_TEST_B( IsEqual(rz,  0.0) );
   ...
   //////////////////////////////////
   // place a hinge support at joing i (Pinned-Fixed)
   ...
   TRY_TEST_B( IsEqual(dx,  0.0) );
   TRY_TEST_B( IsEqual(dy,  0.0) );
   TRY_TEST_B( IsEqual(rz, -0.003125) );
   TRY_TEST_HR(presults->ComputeJointDeflections(lc, 2, &dx, &dy, &rz));
   TRY_TEST_B( IsEqual(dx,  0.0) );
   TRY_TEST_B( IsEqual(dy,  0.0) );
   TRY_TEST_B( IsEqual(rz,  0.0) );
   ...
   ////////////////////////////////
   // make the beam fixed-fixed with a member end release at the start of member 1
   // This will be the same as the pin-fixed beam, except that joint 1 Deflections will be zero
   pJoint1->Support();
   ...
   ////////////////////////////////
   // make the beam fixed-fixed with a member end release at the end of member 1
   // This will be a fix-pin beam
...
*/////////////////////////////////////////////////
   ...
   TRY_TEST_HR(pLoading2->get_MemberStrains(&pMemberStrains));
   CComPtr<IFem2dMemberStrain> pStrainLoad;
   TRY_TEST_LC(pMemberStrains->Create(1, 2, 0.0, -1.0, 2, 1, &pStrainLoad));
