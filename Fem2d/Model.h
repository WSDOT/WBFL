...
/////////////////////////////////////////////////////////////////////////////
// CModel
// Thin COM facade over WBFL::FEA2D::Model. All FE-solution state (the
// stiffness matrix, force vector, joint/member/POI result caches) is gone -
// it lives entirely inside m_pCore now. FEA2D::Model manages its own
// lazy analysis internally (see the design note on Model::EnsureAnalyzed()
// in FEA2D\Model.cpp): every result query transparently refactors the
// stiffness matrix if the structure changed and re-solves only the
// loadings whose loads changed, so this facade doesn't need - and can't
// have, since Analyze() isn't part of FEA2D::Model's public interface any
// more than Compute() was part of the legacy engine's - any dirty-tracking
// of its own. The On*Changed/Added/Removed/*Cleared handlers below exist
// purely to fire the legacy IFem2dModelEvents connection-point events for
// VBA/Excel subscribers; they no longer track dirtiness.
...
private:
   std::unique_ptr<WBFL::FEA2D::Model> m_pCore;
   ...
   // Rebuilds the four top-level COM wrapper collections (and each
   // Loading's five sub-collections) from m_pCore's current contents.
   // Used both by FinalConstruct (core starts empty) and by Load() (core
   // was just wholesale-replaced by m_pCore->Load(), so every pre-existing
   // COM wrapper is bound to a now-dangling core pointer and must be
   // discarded rather than reused).
   HRESULT RebuildComWrappersFromCore();
