...
public:
   ...
   // Wraps an already-existing core Loading (discovered while rehydrating
   // COM wrappers after CModel::Load()). Returns the new CLoading so the
   // caller can walk its five load-type collections and adopt each of
   // their core items too.
   CLoading* AdoptCore(WBFL::FEA2D::Loading* pCore);
