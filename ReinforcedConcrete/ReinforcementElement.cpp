...
//======================== LIFECYCLE  =======================================
rcaReinforcementElement::rcaReinforcementElement(const gmIShape& rShape, 
                                        ...
                                        gpPoint2d* pCg) const
{
   ...
   Float64 area=0.0;
   gmIShapeEx* pex;
   pex = dynamic_cast<gmIShapeEx*>(m_pShape);
   if (pex!=0)
   {
      pex->GetArea(&area,pCg);
   }
   else
{
   gmProperties prop;
...
const gmIShape& rcaReinforcementElement::GetShape() const
{
   ASSERTVALID;
   return *m_pShape;
}
...
gpRect2d rcaReinforcementElement::GetBoundingBox() const
{
   ASSERTVALID;
