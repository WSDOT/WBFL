...
// VoidedSlabEndBlockSegment.cpp : Implementation of CVoidedSlabEndBlockSegment
#include "stdafx.h"
...
HRESULT CVoidedSlabEndBlockSegment::FinalConstruct()
{
   m_Length = 1.0;
...
void CVoidedSlabEndBlockSegment::FinalRelease()
{
   m_Beam.Release();
...
STDMETHODIMP CVoidedSlabEndBlockSegment::InterfaceSupportsErrorInfo(REFIID riid)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::putref_SegmentMeasure(ISegmentMeasure* sm)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::get_Length(Float64 *pVal)
{
   CHECK_RETVAL(pVal);
   (*pVal) = m_Length;
	return S_OK;
...
STDMETHODIMP CVoidedSlabEndBlockSegment::put_Length(Float64 length)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::get_SegmentLength(/*[out, retval]*/ Float64 *pVal)
{
   CHECK_RETVAL(pVal);
   if ( m_Length < 0 )
   {
...
STDMETHODIMP CVoidedSlabEndBlockSegment::get_Shape(Float64 distAlongSegment,IShape** ppShape)
{
   ...
   if ( m_Beam )
   {
      ...
      // set the dimensions
      CComQIPtr<IVoidedSlabSection2> newSection(newShape);
      CComPtr<IVoidedSlab2> newBeam;
      newSection->get_Beam(&newBeam);

      Float64 length;
      get_SegmentLength(&length);

      if (IsLE(distAlongSegment,m_EndBlockLength[etStart]) || 
          IsLE(length - distAlongSegment,m_EndBlockLength[etEnd]) )
      {
...
STDMETHODIMP CVoidedSlabEndBlockSegment::putref_Material(IMaterial* material)
{
   CHECK_IN(material);
...
STDMETHODIMP CVoidedSlabEndBlockSegment::get_Material(IMaterial* *material)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::put_Orientation(Float64 orientation)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::get_Orientation(Float64* orientation)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::Clone(ISegment* *clone)
{
   CHECK_RETOBJ(clone);

   CComObject<CVoidedSlabEndBlockSegment>* pClone;
   CComObject<CVoidedSlabEndBlockSegment>::CreateInstance(&pClone);
   (*clone) = pClone;
   (*clone)->AddRef();
...
////////////////////////////////////////////////////////////////////
// IVoidedSlabEndBlockSegment implementation
STDMETHODIMP CVoidedSlabEndBlockSegment::putref_VoidedSlabSection(/*[in]*/ IVoidedSlabSection2* pPrecastBeam)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::put_EndBlockLength(/*[in]*/EndType endType,/*[in]*/ Float64 length)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::get_EndBlockLength(/*[in]*/EndType endType,/*[out,retval]*/ Float64* pLength)
{
...
STDMETHODIMP CVoidedSlabEndBlockSegment::Load(IStructuredLoad2* load)
{
   return E_NOTIMPL;
   //CComVariant var;
...
STDMETHODIMP CVoidedSlabEndBlockSegment::Save(IStructuredSave2* save)
{
   return E_NOTIMPL;
...
Float64 CVoidedSlabEndBlockSegment::GetSuperstructureMemberLength()
{
   Float64 length;
