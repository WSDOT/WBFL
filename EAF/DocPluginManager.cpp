...
void DocPluginManager::SetSaveMissingPluginDataFlag(BOOL bSaveDataForMissingPlugins)
{
   m_bSaveMissingPluginData = bSaveDataForMissingPlugins;
}

BOOL DocPluginManager::GetSaveMissingPluginDataFlag()
{
   return m_bSaveMissingPluginData;
}

HRESULT DocPluginManager::SavePluginData(IStructuredSave* pStrSave)
{
   if (m_Plugins.size() == 0)
      return S_OK;
   ...
   {
      auto persist = std::dynamic_pointer_cast<IPluginPersist>(plugin);
      if (persist)
      {
         pStrSave->BeginUnit(_T("Plugin"), 1.0);

         CString strCLSID(EAFStringFromCLSID(clsid).c_str());
         pStrSave->put_Property(_T("CLSID"), CComVariant(strCLSID));

         auto hr = persist->Save(pStrSave);
         if (FAILED(hr))
            return hr;
...
HRESULT DocPluginManager::LoadPluginData(IStructuredLoad* pStrLoad)
{
   USES_CONVERSION;
