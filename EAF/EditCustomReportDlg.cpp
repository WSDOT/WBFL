...
void CEditCustomReportDlg::FillChapterData(bool initial)
{
   ...
   if ( sel != CB_ERR )
   {
      ...
      std::shared_ptr<WBFL::ReportMgr::ReportBuilder> pBuilder = pReportMgr->GetReportBuilder(rptname);
      IndexType nc = pBuilder->GetChapterBuilderCount();
      for (IndexType ic = 0; ic<nc; ic++)
      {
         std::shared_ptr<WBFL::ReportMgr::ChapterBuilder> pCb = pBuilder->GetChapterBuilder(ic);
