...
std::unique_ptr<TxnManagerFactory> TxnManager::ms_pFactory;
std::unique_ptr<TxnManager> TxnManager::ms_pInstance;

void TxnManager::Execute(const Transaction& txn)
{
   Execute(std::move(txn.CreateClone()));
}

void TxnManager::Execute(std::unique_ptr<Transaction>&& txn)
{
   if (txn->Execute())
   {
...
void TxnManager::Undo()
{
   ...
   std::unique_ptr<Transaction> txn(std::move(*iter));
...
bool TxnManager::CanRepeat() const
{
   bool retval = false;
...
std::_tstring TxnManager::UndoName() const
{
...
std::_tstring TxnManager::RedoName() const
{
   std::_tstring name(_T(""));
...
std::_tstring TxnManager::RepeatName() const
{
   std::_tstring name(_T(""));
...
IndexType TxnManager::GetTxnCount() const
{
   return m_TxnHistory.size();
}

IndexType TxnManager::GetUndoCount() const
{
   return m_UndoHistory.size();
}

void TxnManager::WriteTransactionLog(std::_tostream& os) const
{
...
void TxnManager::ClearTxnHistory()
{
   m_TxnHistory.clear();
}

void TxnManager::ClearUndoHistory()
{
   m_UndoHistory.clear();
}
...
void TxnManager::SetTransactionManagerFactory(std::unique_ptr<TxnManagerFactory>&& pFactory)
{
   ms_pFactory = std::move(pFactory);
}

TxnManager& TxnManager::GetInstance()
{
   if ( ms_pInstance == nullptr )
   {
      ...
      {
         // we don't have a transcation manager factory so just create the default transaction manager.
         ms_pInstance = std::make_unique<TxnManager>();
      }
      else
      {
         // we have a factory, so let it create the transaction manager.
...
void TxnManager::ShutDown()
{
   // shutting down so release this pointers
   ms_pFactory = nullptr;
   ms_pInstance = nullptr;
}

void TxnManager::WriteLogIntroduction(std::_tostream& /*os*/) const
{
   // Do nothing
}

void TxnManager::WriteLogConclusion(std::_tostream& /*os*/) const
{
   // Do nothing
}

TxnManager::TxnContainer::iterator TxnManager::FindFirstUndoableTxn()
{
...
TxnManager::TxnContainer::const_iterator TxnManager::FindFirstUndoableTxn() const
{
